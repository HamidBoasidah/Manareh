			...
			<div class="p-4 sm:p-6 dark:border-gray-800">
				<div class="grid grid-cols-1 gap-5 md:grid-cols-2">
					<div class="md:col-span-2">
						<label for="program-name" class="mb-1.5 block text-sm font-medium text-gray-700 dark:text-gray-400">
							{{ t('programs.name') }}
						</label>
						<input
							v-model="form.name"
					...
					<div>
						<label for="program-type" class="mb-1.5 block text-sm font-medium text-gray-700 dark:text-gray-400">
							{{ t('programs.type') }}
						</label>
						<input
							v-model="form.type"
							...
							id="program-type"
							autocomplete="off"
							:placeholder="t('programs.typePlaceholder')"
							class="dark:bg-dark-900 shadow-theme-xs focus:border-brand-300 focus:ring-brand-500/10 dark:focus:border-brand-800 h-11 w-full rounded-lg border border-gray-300 bg-transparent px-4 py-2.5 text-sm text-gray-800 placeholder:text-gray-400 focus:ring-3 focus:outline-hidden dark:border-gray-700 dark:bg-gray-900 dark:text-white/90 dark:placeholder:text-white/30"
						/>
					...
					<div class="md:col-span-2">
						<label for="program-description" class="mb-1.5 block text-sm font-medium text-gray-700 dark:text-gray-400">
...
const form = useForm({
	...
	mosque_id: props.program.mosque_id ?? '',
	description: props.program.description ?? '',
	is_active: Boolean(props.program.is_active),
})

function update() {
	form.put(route('admin.programs.update', props.program.id), {
		onSuccess: () => {
			success(t('programs.programUpdatedSuccessfully'))
		},
		onError: () => {
			error(t('programs.programUpdateFailed'))
		},
		preserveScroll: true,
	})
}
