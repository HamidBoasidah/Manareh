<template>
			...
			<table class="w-full min-w-full">
					...
					<tr>
						...
						<th class="px-4 py-3 text-start border border-gray-100 dark:border-gray-800">
							<div class="flex items-center justify-between w-full cursor-pointer" @click="sortBy('name')">
								<p class="font-medium text-gray-700 text-theme-xs dark:text-gray-400">{{ t('programs.name') }}</p>
						...
						<th class="px-4 py-3 text-start border border-gray-100 dark:border-gray-800">
							<p class="font-medium text-gray-700 text-theme-xs dark:text-gray-400">{{ t('programs.mosque') }}</p>
						</th>

						<th class="px-4 py-3 text-start border border-gray-100 dark:border-gray-800">
							<div class="flex items-center justify-between w-full cursor-pointer" @click="sortBy('type')">
								<p class="font-medium text-gray-700 text-theme-xs dark:text-gray-400">{{ t('programs.type') }}</p>
								<span class="flex flex-col gap-0.5">
									<svg class="fill-gray-300 dark:fill-gray-700" width="8" height="5" viewBox="0 0 8 5" fill="none"><path d="M4.40962 0.585167C4.21057 0.300808 3.78943 0.300807 3.59038 0.585166L1.05071 4.21327C0.81874 4.54466 1.05582 5 1.46033 5H6.53967C6.94418 5 7.18126 4.54466 6.94929 4.21327L4.40962 0.585167Z" /></svg>
									<svg class="fill-gray-300 dark:fill-gray-700" width="8" height="5" viewBox="0 0 8 5" fill="none"><path d="M4.40962 4.41483C4.21057 4.69919 3.78943 4.69919 3.59038 4.41483L1.05071 0.786732C0.81874 0.455343 1.05582 0 1.46033 0H6.53967C6.94418 0 7.18126 0.455342 6.94929 0.786731L4.40962 4.41483Z" /></svg>
								</span>
				...
				<tbody>
					<tr
						...
						:key="program.id"
						:class="{ 'bg-gray-50 dark:bg-gray-900': program.selected }"
					>
						<td class="px-4 py-3 border border-gray-100 dark:border-gray-800">
							<label class="flex items-center text-sm font-medium text-gray-700 cursor-pointer select-none dark:text-gray-400">
								<span class="relative">
									<input type="checkbox" class="sr-only" v-model="program.selected" @change="updateSelectAll" />
									<span
										:class="program.selected ? 'border-brand-500 bg-brand-500' : 'bg-transparent border-gray-300 dark:border-gray-700'"
										class="flex h-4 w-4 items-center justify-center rounded-sm border-[1.25px]"
									>
										<span :class="program.selected ? '' : 'opacity-0'">
											<svg width="12" height="12" viewBox="0 0 12 12" fill="none">
												<path d="M10 3L4.5 8.5L2 6" stroke="white" stroke-width="1.6666" stroke-linecap="round" stroke-linejoin="round" />
											</svg>
										</span>
						...
						<td class="px-4 py-3 border border-gray-100 dark:border-gray-800">
							<p class="text-gray-700 text-theme-sm dark:text-gray-400">{{ program.name }}</p>
						</td>

						<td class="px-4 py-3 border border-gray-100 dark:border-gray-800">
							<p class="text-gray-700 text-theme-sm dark:text-gray-400">{{ program.mosque_name ?? '—' }}</p>
						</td>

						<td class="px-4 py-3 border border-gray-100 dark:border-gray-800">
							<p class="text-gray-700 text-theme-sm dark:text-gray-400">{{ program.type ?? '—' }}</p>
						</td>
						...
						<td class="px-4 py-3 border border-gray-100 dark:border-gray-800">
							<span
								...
								:class="{
									'bg-green-50 text-green-600 dark:bg-green-500/15 dark:text-green-500': program.is_active,
									'bg-error-50 text-error-600 dark:bg-error-500/15 dark:text-error-500': !program.is_active,
								}"
							>
						...
						<td class="px-4 py-3 border border-gray-100 dark:border-gray-800">
							<label :for="'toggle-' + program.id" class="cursor-pointer">
								<div class="relative">
									<input
										type="checkbox"
										...
										@change="toggleProgramStatus(program)"
									/>
									<div class="block h-5 w-9 rounded-full" :class="program.is_active ? 'bg-brand-500 dark:bg-brand-500' : 'bg-gray-200 dark:bg-white/10'"></div>
									<div :class="[program.is_active ? 'rtl:translate-x-[-100%] ltr:translate-x-full' : 'translate-x-0']" class="shadow-theme-sm absolute top-0.5 h-4 w-4 rounded-full bg-white duration-200 ease-linear rtl:right-0.5 ltr:left-0.5"></div>
								</div>
							</label>
						</td>
						...
						<td class="px-4 py-3 border border-gray-100 dark:border-gray-800">
							<div class="flex items-center w-full gap-2">
								<Tooltip :text="t('messages.notAuthorized')" :show="!canView">
									<button
										:disabled="!canView"
										@click="handleViewClick(program.id)"
								...
								<Tooltip :text="t('messages.notAuthorized')" :show="!canEdit">
									<button
										:disabled="!canEdit"
										@click="handleEditClick(program.id)"
								...
								<Tooltip :text="t('messages.notAuthorized')" :show="!canDelete">
									<button
										:disabled="!canDelete"
										@click="handleDeleteClick(program.id)"
	...
	<DangerAlert
		...
		:title="t('messages.areYouSure')"
			:message="t('programs.deleteProgramConfirmation')"
...
const canCreate = computed(() => hasAnyPermission(['programs.create', 'programs.store', 'programs.add']))
const canView = computed(() => hasAnyPermission(['programs.view', 'programs.show', 'programs.read']))
const canEdit = computed(() => hasAnyPermission(['programs.update', 'programs.edit']))
const canDelete = computed(() => hasAnyPermission(['programs.delete', 'programs.destroy']))
...
const currentPage = ref(props.programs?.current_page ?? 1)
const perPage = ref(props.programs?.per_page ?? 10)
...
function goToView(id) {
	router.visit(route('admin.programs.show', id))
}

function goToEdit(id) {
	router.visit(route('admin.programs.edit', id))
}
...
const programToDeleteId = ref(null)

function openDeleteModal(id) {
	programToDeleteId.value = id
...
function closeDeleteModal() {
	...
	programToDeleteId.value = null
}

function confirmDelete() {
	if (!programToDeleteId.value) return

	router.delete(route('admin.programs.destroy', programToDeleteId.value), {
		onSuccess: () => {
			success(t('programs.programDeletedSuccessfully'))
			closeDeleteModal()
		},
		onError: () => {
			error(t('programs.programDeletionFailed'))
			closeDeleteModal()
		},
		preserveScroll: true,
	})
...
function toggleProgramStatus(program) {
	const wasActive = program.is_active
	program.is_active = !wasActive

	const url = wasActive
		? route('admin.programs.deactivate', { id: program.id })
		: route('admin.programs.activate', { id: program.id })
...
const filteredData = computed(() => {
	...
	return (props.programs?.data || [])
		.filter((program) => {
			...
			const mosque = program.mosque_name?.toLowerCase() || ''
			return name.includes(searchLower) || type.includes(searchLower) || mosque.includes(searchLower)
		})
		.sort((a, b) => {
...
const totalEntries = computed(() => props.programs?.total || filteredData.value.length)
const startEntry = computed(() => props.programs?.from || 1)
const endEntry = computed(() => props.programs?.to || filteredData.value.length)
const totalPages = computed(() => props.programs?.last_page || 1)
...
watch(() => props.programs?.current_page, (val) => {
	currentPage.value = typeof val === 'number' ? val : 1
})

watch(() => props.programs?.per_page, (val) => {
...
const toggleSelectAll = () => {
	filteredData.value.forEach((program) => {
		program.selected = selectAll.value
	})
}

const updateSelectAll = () => {
	const items = filteredData.value
	selectAll.value = items.length > 0 && items.every((program) => program.selected)
}
