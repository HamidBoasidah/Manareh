...
const form = useForm({
  _method: 'PUT',
  name_ar: props.district.name_ar ?? '',
  name_en: props.district.name_en ?? '',
  governorate_id: props.district.governorate_id ?? '',
  is_active: !!props.district.is_active,
})

function update() {
  form.post(route('admin.districts.update', props.district.id), {
    onSuccess: () => {
      success(t('districts.districtUpdatedSuccessfully'))
    },
    onError: () => {
      error(t('districts.districtUpdateFailed'))
    },
    preserveScroll: true,
  })
}
