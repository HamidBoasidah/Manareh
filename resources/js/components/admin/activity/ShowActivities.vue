<template>
      ...
      <table class="w-full min-w-full">
          ...
          <tr>
            ...
            <th class="px-4 py-3 text-start border border-gray-100 dark:border-gray-800">
              <div class="flex items-center justify-between w-full cursor-pointer" @click="sortBy('title')">
                <p class="font-medium text-gray-700 text-theme-xs dark:text-gray-400">{{ t('activities.title') }}</p>
            ...
            <th class="px-4 py-3 text-start border border-gray-100 dark:border-gray-800">
              <div class="flex items-center justify-between w-full cursor-pointer" @click="sortBy('mosque_name')">
                <p class="font-medium text-gray-700 text-theme-xs dark:text-gray-400">{{ t('activities.mosqueName') }}</p>
                <span class="flex flex-col gap-0.5">
                  <svg class="fill-gray-300 dark:fill-gray-700" width="8" height="5" viewBox="0 0 8 5" fill="none"><path d="M4.40962 0.585167C4.21057 0.300808 3.78943 0.300807 3.59038 0.585166L1.05071 4.21327C0.81874 4.54466 1.05582 5 1.46033 5H6.53967C6.94418 5 7.18126 4.54466 6.94929 4.21327L4.40962 0.585167Z" /></svg>
                  <svg class="fill-gray-300 dark:fill-gray-700" width="8" height="5" viewBox="0 0 8 5" fill="none"><path d="M4.40962 4.41483C4.21057 4.69919 3.78943 4.69919 3.59038 4.41483L1.05071 0.786732C0.81874 0.455343 1.05582 0 1.46033 0H6.53967C6.94418 0 7.18126 0 6.94929 0.786731L4.40962 4.41483Z" /></svg>
                </span>
            ...
            <th class="px-4 py-3 text-start border border-gray-100 dark:border-gray-800">
              <div class="flex items-center justify-between w-full cursor-pointer" @click="sortBy('activity_date_g')">
                <p class="font-medium text-gray-700 text-theme-xs dark:text-gray-400">{{ t('activities.activityDate') }}</p>
                <span class="flex flex-col gap-0.5">
                  <svg class="fill-gray-300 dark:fill-gray-700" width="8" height="5" viewBox="0 0 8 5" fill="none"><path d="M4.40962 0.585167C4.21057 0.300808 3.78943 0.300807 3.59038 0.585166L1.05071 4.21327C0.81874 4.54466 1.05582 5 1.46033 5H6.53967C6.94418 5 7.18126 4.54466 6.94929 4.21327L4.40962 0.585167Z" /></svg>
                  <svg class="fill-gray-300 dark:fill-gray-700" width="8" height="5" viewBox="0 0 8 5" fill="none"><path d="M4.40962 4.41483C4.21057 4.69919 3.78943 4.69919 3.59038 4.41483L1.05071 0.786732C0.81874 0.455343 1.05582 0 1.46033 0H6.53967C6.94418 0 7.18126 0 6.94929 0.786731L4.40962 4.41483Z" /></svg>
                </span>
            ...
            <th class="px-4 py-3 text-start border border-gray-100 dark:border-gray-800">
              <div class="flex items-center justify-between w-full cursor-pointer" @click="sortBy('place')">
                <p class="font-medium text-gray-700 text-theme-xs dark:text-gray-400">{{ t('activities.place') }}</p>
                <span class="flex flex-col gap-0.5">
                  <svg class="fill-gray-300 dark:fill-gray-700" width="8" height="5" viewBox="0 0 8 5" fill="none"><path d="M4.40962 0.585167C4.21057 0.300808 3.78943 0.300807 3.59038 0.585166L1.05071 4.21327C0.81874 4.54466 1.05582 5 1.46033 5H6.53967C6.94418 5 7.18126 4.54466 6.94929 4.21327L4.40962 0.585167Z" /></svg>
                  <svg class="fill-gray-300 dark:fill-gray-700" width="8" height="5" viewBox="0 0 8 5" fill="none"><path d="M4.40962 4.41483C4.21057 4.69919 3.78943 4.69919 3.59038 4.41483L1.05071 0.786732C0.81874 0.455343 1.05582 0 1.46033 0H6.53967C6.94418 0 7.18126 0 6.94929 0.786731L4.40962 4.41483Z" /></svg>
                </span>
        ...
        <tbody>
          <tr
            ...
            :key="activity.id"
            :class="{ 'bg-gray-50 dark:bg-gray-900': activity.selected }"
          >
            <td class="px-4 py-3 border border-gray-100 dark:border-gray-800">
              <label class="flex items-center text-sm font-medium text-gray-700 cursor-pointer select-none dark:text-gray-400">
                <span class="relative">
                  <input type="checkbox" class="sr-only" v-model="activity.selected" @change="updateSelectAll" />
                  <span
                    :class="activity.selected ? 'border-brand-500 bg-brand-500' : 'bg-transparent border-gray-300 dark:border-gray-700'"
                    class="flex h-4 w-4 items-center justify-center rounded-sm border-[1.25px]"
                  >
                    <span :class="activity.selected ? '' : 'opacity-0'">
            ...
            <td class="px-4 py-3 border border-gray-100 dark:border-gray-800">
              <p class="text-gray-700 text-theme-sm dark:text-gray-400">{{ activity.title }}</p>
            </td>

            <td class="px-4 py-3 border border-gray-100 dark:border-gray-800">
              <p class="text-gray-700 text-theme-sm dark:text-gray-400">{{ activity.mosque_name || '—' }}</p>
            </td>

            <td class="px-4 py-3 border border-gray-100 dark:border-gray-800">
              <p class="text-gray-700 text-theme-sm dark:text-gray-400">{{ formatDate(activity.activity_date_g) }}</p>
            </td>

            <td class="px-4 py-3 border border-gray-100 dark:border-gray-800">
              <p class="text-gray-700 text-theme-sm dark:text-gray-400">{{ activity.place || '—' }}</p>
            </td>
            ...
            <td class="px-4 py-3 border border-gray-100 dark:border-gray-800">
              <label :for="'toggle-' + activity.id" class="cursor-pointer">
                <div class="relative">
                  <input
                    type="checkbox"
                    ...
                    @change="toggleActivityStatus(activity)"
                  />
                  <div class="block h-5 w-9 rounded-full" :class="activity.is_active ? 'bg-brand-500 dark:bg-brand-500' : 'bg-gray-200 dark:bg-white/10'"></div>
                  <div :class="[activity.is_active ? 'rtl:translate-x-[-100%] ltr:translate-x-full' : 'translate-x-0']" class="shadow-theme-sm absolute top-0.5 h-4 w-4 rounded-full bg-white duration-200 ease-linear rtl:right-0.5 ltr:left-0.5"></div>
                </div>
              </label>
            </td>
            ...
            <td class="px-4 py-3 border border-gray-100 dark:border-gray-800">
              <div class="flex items-center w-full gap-2">
                <Tooltip :text="t('messages.notAuthorized')" :show="!canView">
                  <button
                    :disabled="!canView"
                    @click="handleViewClick(activity.id)"
                ...
                <Tooltip :text="t('messages.notAuthorized')" :show="!canEdit">
                  <button
                    :disabled="!canEdit"
                    @click="handleEditClick(activity.id)"
                ...
                <Tooltip :text="t('messages.notAuthorized')" :show="!canDelete">
                  <button
                    :disabled="!canDelete"
                    @click="handleDeleteClick(activity.id)"
  ...
  <DangerAlert
    ...
    :message="t('messages.deleteActivityConfirmation')"
...
const canCreate = computed(() => hasAnyPermission(['activities.create', 'activities.store', 'activities.add']))
const canView   = computed(() => hasAnyPermission(['activities.view', 'activities.show', 'activities.read']))
const canEdit   = computed(() => hasAnyPermission(['activities.update', 'activities.edit']))
const canDelete = computed(() => hasAnyPermission(['activities.delete', 'activities.destroy']))
...
const perPage        = ref(props.activities?.per_page ?? 10)
const selectAll      = ref(false)

function goToCreate()      { router.visit(route('admin.activities.create')) }
function goToView(id)      { router.visit(route('admin.activities.show', id)) }
function goToEdit(id)      { router.visit(route('admin.activities.edit', id)) }
...
const activityToDeleteId = ref(null)

function openDeleteModal(id)  { activityToDeleteId.value = id; isDeleteModalOpen.value = true }
function handleDeleteClick(id){ if (!canDelete.value) return; openDeleteModal(id) }
function closeDeleteModal()   { isDeleteModalOpen.value = false; activityToDeleteId.value = null }

function confirmDelete() {
  if (activityToDeleteId.value) {
    router.delete(route('admin.activities.destroy', activityToDeleteId.value), {
      onSuccess: () => { success(t('activities.activityDeletedSuccessfully')); closeDeleteModal() },
      onError:   () => { error(t('activities.activityDeletionFailed'));       closeDeleteModal() },
      preserveScroll: true,
    })
  }
}
...
function toggleActivityStatus(activity) {
  const wasActive = activity.is_active
  activity.is_active = !wasActive
  const url = wasActive
    ? route('admin.activities.deactivate', { id: activity.id })
    : route('admin.activities.activate',   { id: activity.id })
  router.patch(url, {}, {
    preserveState: true,
    preserveScroll: true,
    onError: () => { activity.is_active = wasActive },
  })
}
...
const filteredData = computed(() => {
  ...
  return (props.activities?.data || [])
    .filter((d) => {
      ...
      return title.includes(searchLower) || mosque.includes(searchLower) || place.includes(searchLower)
    })
    .sort((a, b) => {
...
const totalEntries = computed(() => props.activities?.total || filteredData.value.length)
const startEntry   = computed(() => props.activities?.from  || 1)
const endEntry     = computed(() => props.activities?.to    || filteredData.value.length)
const totalPages   = computed(() => props.activities?.last_page || 1)
