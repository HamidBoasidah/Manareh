<template>
  <form class="space-y-6" @submit.prevent="create">
      ...
      <div class="p-4 sm:p-6 dark:border-gray-800">
        <div class="grid grid-cols-1 gap-5 md:grid-cols-2">
          <div>
            ...
            <input v-model="form.title" type="text" autocomplete="off" :placeholder="t('activities.title')" class="h-11 w-full rounded-lg border border-gray-300 px-4" />
          ...
          <div>
            ...
            <div class="relative z-20 bg-transparent">
              <select v-model="form.mosque_id" class="h-11 w-full rounded-lg border border-gray-300 px-4">
                <option value="" disabled>{{ t('activities.selectMosque') || t('activities.mosqueName') }}</option>
                <option v-for="m in mosques || []" :key="m.id" :value="m.id">{{ m.name }}</option>
              </select>
            </div>
          ...
          <div>
            ...
            <input v-model="form.place" type="text" autocomplete="off" :placeholder="t('activities.place')" class="h-11 w-full rounded-lg border border-gray-300 px-4" />
      ...
      <div class="p-4 sm:p-6">
        <label class="flex cursor-pointer select-none items-center gap-3 text-sm font-medium text-gray-700 dark:text-gray-400">
          <div class="relative">
            ...
            <div :class="[ 'block h-6 w-11 rounded-full', form.is_active ? 'bg-brand-500' : 'bg-gray-200' ]"></div>
            <div :class="[ form.is_active ? 'translate-x-full' : 'translate-x-0', 'absolute left-0.5 top-0.5 h-5 w-5 rounded-full bg-white' ]"></div>
          </div>
          <span :class="[ 'inline-flex items-center justify-center gap-1 rounded-full px-2.5 py-0.5 text-xs font-medium', form.is_active ? 'bg-green-50 text-green-600' : 'bg-error-50 text-error-600' ]">{{ form.is_active ? t('common.active') : t('common.inactive') }}</span>
        </label>
      </div>
    </div>
    ...
    <div class="flex flex-col gap-3 sm:flex-row sm:justify-end">
      ...
      <button type="submit" class="bg-brand-500 shadow-theme-xs hover:bg-brand-600 inline-flex items-center justify-center gap-2 rounded-lg px-4 py-3 text-sm font-medium text-white" :disabled="form.processing">{{ form.processing ? t('common.loading') : t('buttons.create') }}</button>
    </div>
  </form>
</template>
...
const form = useForm({ title: '', description: '', activity_date_g: '', place: '', mosque_id: '', is_active: true })

function create() {
  form.post(route('admin.activities.store'), {
    onSuccess: () => success(t('activities.activityCreatedSuccessfully')),
