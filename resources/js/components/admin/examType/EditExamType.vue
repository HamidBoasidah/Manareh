  ...
  <form class="space-y-6" @submit.prevent="update">
    <div class="rounded-2xl border border-gray-200 bg-white dark:border-gray-800 dark:bg-white/[0.03]">
      <div class="border-b border-gray-200 px-6 py-4 dark:border-gray-800">
        <h2 class="text-lg font-medium text-gray-800 dark:text-white">
          {{ t('exam_types.examTypeInformation') }}
        </h2>
      </div>
      ...
      <div class="p-4 sm:p-6 dark:border-gray-800">
        <div class="grid grid-cols-1 gap-5 md:grid-cols-2">
          <div>
            <label for="exam-type-name" class="mb-1.5 block text-sm font-medium text-gray-700 dark:text-gray-400">
              {{ t('exam_types.name') }}
            </label>
            <input
              v-model="form.name"
          ...
          <div>
            ...
            <div class="relative z-20 bg-transparent">
              ...
              >
                ...
                >
                  {{ locale === 'ar' ? mosque.name_ar : mosque.name_en }}
                </option>
              </select>
    ...
    <div class="rounded-2xl border border-gray-200 bg-white dark:border-gray-800 dark:bg-white/[0.03]">
      ...
      <div class="p-4 sm:p-6">
        <label for="exam-type-active" class="flex cursor-pointer select-none items-center gap-3 text-sm font-medium text-gray-700 dark:text-gray-400">
          <div class="relative">
            <input type="checkbox" id="exam-type-active" class="sr-only" v-model="form.is_active" />
    ...
    <div class="flex flex-col gap-3 sm:flex-row sm:justify-end">
      <Link
        :href="route('admin.exam_types.index')"
...
const form = useForm({
  ...
  mosque_id: props.examType.mosque_id ?? '',
  is_active: !!props.examType.is_active,
})

function update() {
  form.post(route('admin.exam_types.update', props.examType.id), {
    onSuccess: () => {
      success(t('exam_types.examTypeUpdatedSuccessfully'))
    },
    onError: () => {
      error(t('exam_types.examTypeUpdateFailed'))
    },
    preserveScroll: true,
  })
}
