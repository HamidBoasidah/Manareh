<template>
  <div class="space-y-6">
        ...
        <form @submit.prevent>
          <div class="grid grid-cols-1 gap-5 md:grid-cols-2">
            <div>
              ...
              <input v-model="form.name" type="text" id="mosque-name" autocomplete="off" class="dark:bg-dark-900 shadow-theme-xs focus:border-brand-300 focus:ring-brand-500/10 dark:focus:border-brand-800 h-11 w-full rounded-lg border border-gray-300 bg-transparent px-4 py-2.5 text-sm text-gray-800 placeholder:text-gray-400 focus:ring-3 focus:outline-hidden dark:border-gray-700 dark:bg-gray-900 dark:text-white/90 dark:placeholder:text-white/30" />
            </div>

            <div>
              <label for="mosque-city" class="mb-1.5 block text-sm font-medium text-gray-700 dark:text-gray-400">{{ t('common.city') }}</label>
              <input v-model="form.city" type="text" id="mosque-city" autocomplete="off" class="dark:bg-dark-900 shadow-theme-xs focus:border-brand-300 focus:ring-brand-500/10 dark:focus:border-brand-800 h-11 w-full rounded-lg border border-gray-300 bg-transparent px-4 py-2.5 text-sm text-gray-800 placeholder:text-gray-400 focus:ring-3 focus:outline-hidden dark:border-gray-700 dark:bg-gray-900 dark:text-white/90 dark:placeholder:text-white/30" />
            </div>
            ...
            <div class="md:col-span-2">
              ...
              <input v-model="form.address" type="text" id="mosque-address" autocomplete="off" class="dark:bg-dark-900 shadow-theme-xs focus:border-brand-300 focus:ring-brand-500/10 dark:focus:border-brand-800 h-11 w-full rounded-lg border border-gray-300 bg-transparent px-4 py-2.5 text-sm text-gray-800 placeholder:text-gray-400 focus:ring-3 focus:outline-hidden dark:border-gray-700 dark:bg-gray-900 dark:text-white/90 dark:placeholder:text-white/30" />
            </div>
            ...
            <div>
              ...
              <input v-model="form.phone" type="text" id="mosque-phone" autocomplete="off" class="dark:bg-dark-900 shadow-theme-xs focus:border-brand-300 focus:ring-brand-500/10 dark:focus:border-brand-800 h-11 w-full rounded-lg border border-gray-300 bg-transparent px-4 py-2.5 text-sm text-gray-800 placeholder:text-gray-400 focus:ring-3 focus:outline-hidden dark:border-gray-700 dark:bg-gray-900 dark:text-white/90 dark:placeholder:text-white/30" />
            </div>
    ...
    <div class="flex flex-col gap-3 sm:flex-row sm:justify-end">
      ...
      <button @click="update" class="bg-brand-500 shadow-theme-xs hover:bg-brand-600 inline-flex items-center justify-center gap-2 rounded-lg px-4 py-3 text-sm font-medium text-white transition">{{ t('buttons.update') }}</button>
    </div>
    </div>
</template>
...
const props = defineProps({ mosque: Object })

const form = useForm({ _method: 'PUT', name: props.mosque.name, city: props.mosque.city, address: props.mosque.address, phone: props.mosque.phone, notes: props.mosque.notes, is_active: props.mosque.is_active })

function update() {
  form.post(route('admin.mosques.update', props.mosque.id), {
    onSuccess: () => { success(t('mosques.mosqueUpdatedSuccessfully')) },
    onError: () => { error(t('mosques.mosqueUpdateFailed')) },
    preserveScroll: true,
  })
}
</script>
