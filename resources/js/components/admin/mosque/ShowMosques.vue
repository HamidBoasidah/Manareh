  ...
  <div class="overflow-hidden">
    ...
    <div class="max-w-full overflow-x-auto">
      <table class="w-full min-w-full">
          ...
          <tr>
            <th class="px-4 py-3 text-start border border-gray-100 dark:border-gray-800 w-12">
              <label class="flex items-center text-sm font-medium text-gray-700 cursor-pointer select-none dark:text-gray-400">
                <span class="relative">
                  ...
                  <span :class="selectAll ? 'border-brand-500 bg-brand-500' : 'bg-transparent border-gray-300 dark:border-gray-700'" class="flex h-4 w-4 items-center justify-center rounded-sm border-[1.25px]"><span :class="selectAll ? '' : 'opacity-0'">✔</span></span>
                </span>
              </label>
            </th>
            <th class="px-4 py-3 text-start border border-gray-100 dark:border-gray-800">
              <div class="flex items-center justify-between w-full cursor-pointer" @click="sortBy('name')">
                <p class="font-medium text-gray-700 text-theme-xs dark:text-gray-400">{{ t('mosques.name') }}</p>
              </div>
            </th>
            <th class="px-4 py-3 text-start border border-gray-100 dark:border-gray-800">
              <p class="font-medium text-gray-700 text-theme-xs dark:text-gray-400">{{ t('common.city') }}</p>
            </th>
            <th class="px-4 py-3 text-start border border-gray-100 dark:border-gray-800">
        ...
        <tbody>
          <tr v-for="mosque in paginatedData" :key="mosque.id" :class="{ 'bg-gray-50 dark:bg-gray-900': mosque.selected }">
            <td class="px-4 py-3 border border-gray-100 dark:border-gray-800">
              <label class="flex items-center text-sm font-medium text-gray-700 cursor-pointer select-none dark:text-gray-400">
                <span class="relative">
                  <input type="checkbox" class="sr-only" v-model="mosque.selected" @change="updateSelectAll" />
                  <span :class="mosque.selected ? 'border-brand-500 bg-brand-500' : 'bg-transparent border-gray-300 dark:border-gray-700'" class="flex h-4 w-4 items-center justify-center rounded-sm border-[1.25px]"><span :class="mosque.selected ? '' : 'opacity-0'">✔</span></span>
                </span>
              </label>
            </td>
            <td class="px-4 py-3 border border-gray-100 dark:border-gray-800"><p class="text-gray-700 text-theme-sm dark:text-gray-400">{{ mosque.name }}</p></td>
            <td class="px-4 py-3 border border-gray-100 dark:border-gray-800"><p class="text-gray-700 text-theme-sm dark:text-gray-400">{{ mosque.city }}</p></td>
            <td class="px-4 py-3 border border-gray-100 dark:border-gray-800">
              <span class="inline-flex items-center justify-center gap-1 rounded-full px-2.5 py-0.5 text-xs font-medium" :class="{ 'bg-green-50 text-green-600 dark:bg-green-500/15 dark:text-green-500': mosque.is_active, 'bg-error-50 text-error-600 dark:bg-error-500/15 dark:text-error-500': !mosque.is_active }">{{ mosque.is_active ? t('common.active') : t('common.inactive') }}</span>
            </td>
            <td class="px-4 py-3 border border-gray-100 dark:border-gray-800">
                <label :for="'toggle-' + mosque.id" class="cursor-pointer">
                    <div class="relative">
                        <input
                            type="checkbox"
                            :id="'toggle-' + mosque.id"
                            class="sr-only"
                            :checked="mosque.is_active"
                            @change="toggleMosqueStatus(mosque)"
                        />
                        <div
                            class="block h-5 w-9 rounded-full"
                            :class="mosque.is_active ? 'bg-brand-500 dark:bg-brand-500' : 'bg-gray-200 dark:bg-white/10'"
                        ></div>
                        <div
                            :class="[mosque.is_active ? 'rtl:translate-x-[-100%] ltr:translate-x-full' : 'translate-x-0']"
                ...
                <td class="px-4 py-3 border border-gray-100 dark:border-gray-800">
                    <div class="flex items-center w-full gap-2">
                        <Tooltip :text="t('messages.notAuthorized')" :show="!canView">
                            <button
                                :disabled="!canView"
                                @click="handleViewClick(mosque.id)"
                        ...
                        <Tooltip :text="t('messages.notAuthorized')" :show="!canEdit">
                            <button
                                :disabled="!canEdit"
                                @click="handleEditClick(mosque.id)"
                        ...
                        <Tooltip :text="t('messages.notAuthorized')" :show="!canDelete">
                            <button
                                :disabled="!canDelete"
                                @click="handleDeleteClick(mosque.id)"
          ...
          <tr v-if="paginatedData.length === 0">
            <td colspan="6" class="px-4 py-6 text-center text-sm text-gray-500 dark:text-gray-400">{{ t('mosques.noMosque') }}</td>
          </tr>
        </tbody>
      </table>
    </div>
    ...
    <DangerAlert :isOpen="isDeleteModalOpen" :title="t('messages.areYouSure')" :message="t('messages.deleteMosqueConfirmation')" @close="closeDeleteModal" @confirm="confirmDelete" />
  </div>
</template>
...
const canCreate = computed(() => hasAnyPermission(['mosques.create', 'mosques.store', 'mosques.add']))
const canView = computed(() => hasAnyPermission(['mosques.view', 'mosques.show', 'mosques.read']))
const canEdit = computed(() => hasAnyPermission(['mosques.update', 'mosques.edit']))
const canDelete = computed(() => hasAnyPermission(['mosques.delete', 'mosques.destroy']))

const { t } = useI18n()
const { success, error } = useNotifications()

const props = defineProps({ mosques: Object })

const search = ref('')
const sortColumn = ref('name')
const sortDirection = ref('asc')
const currentPage = ref(props.mosques?.current_page ?? 1)
const perPage = ref(props.mosques?.per_page ?? 10)
const selectAll = ref(false)

function goToCreate() { router.visit(route('admin.mosques.create')) }
function goToView(id) { router.visit(route('admin.mosques.show', id)) }
function goToEdit(id) { router.visit(route('admin.mosques.edit', id)) }
...
const mosqueToDeleteId = ref(null)

function openDeleteModal(mosqueId) { mosqueToDeleteId.value = mosqueId; isDeleteModalOpen.value = true }
function handleDeleteClick(mosqueId) { if (!canDelete.value) return; openDeleteModal(mosqueId) }
function closeDeleteModal() { isDeleteModalOpen.value = false; mosqueToDeleteId.value = null }

function confirmDelete() {
  if (mosqueToDeleteId.value) {
    router.delete(route('admin.mosques.destroy', mosqueToDeleteId.value), {
      onSuccess: () => { success(t('mosques.mosqueDeletedSuccessfully')); closeDeleteModal() },
      onError: () => { error(t('mosques.mosqueDeletionFailed')); closeDeleteModal() },
      preserveScroll: true,
    })
  }
}

function toggleMosqueStatus(mosque) {
    const wasActive = mosque.is_active
    mosque.is_active = !wasActive
    const url = wasActive
        ? route('admin.mosques.deactivate', { id: mosque.id })
        : route('admin.mosques.activate', { id: mosque.id })
    router.patch(url, {}, {
        preserveState: true,
        preserveScroll: true,
        onError: () => {
...
const filteredData = computed(() => {
  ...
  return (props.mosques?.data || []).filter((m) => {
...
const totalEntries = computed(() => props.mosques?.total || filteredData.value.length)
const startEntry = computed(() => props.mosques?.from || 1)
const endEntry = computed(() => props.mosques?.to || filteredData.value.length)
const totalPages = computed(() => props.mosques?.last_page || 1)
...
watch(() => props.mosques?.current_page, (val) => { currentPage.value = typeof val === 'number' ? val : 1 })
watch(() => props.mosques?.per_page, (val) => { if (typeof val === 'number') perPage.value = val })
...
const toggleSelectAll = () => { filteredData.value.forEach((m) => { m.selected = selectAll.value }) }
const updateSelectAll = () => { const items = filteredData.value; selectAll.value = items.length > 0 && items.every((m) => m.selected) }
