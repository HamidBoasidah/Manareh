...
const form = useForm({
  _method: 'PUT',
  circle_id: props.teacherAttendance.circle_id ?? '',
  user_id: props.teacherAttendance.user_id ?? '',
  date_g: props.teacherAttendance.date_g ?? '',
  date_h: props.teacherAttendance.date_h ?? '',
  status: props.teacherAttendance.status ?? '',
  notes: props.teacherAttendance.notes ?? '',
  recorded_by: props.teacherAttendance.recorded_by ?? '',
  is_active: !!props.teacherAttendance.is_active,
})

function update() {
  form.post(route('admin.teacher_attendances.update', props.teacherAttendance.id), {
    onSuccess: () => {
      success(t('teacher_attendances.teacherAttendanceUpdatedSuccessfully'))
    },
    onError: () => {
      error(t('teacher_attendances.teacherAttendanceUpdateFailed'))
    },
    preserveScroll: true,
  })
}
