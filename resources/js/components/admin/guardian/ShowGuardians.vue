<template>
  <div class="overflow-hidden">
    ...
    <div class="max-w-full overflow-x-auto">
      <table class="w-full min-w-full">
        <thead>
          <tr>
            ...
            <th class="px-4 py-3 text-start border border-gray-100 dark:border-gray-800">
              <div
                class="flex items-center justify-between w-full cursor-pointer"
                @click="sortBy('relation')"
              >
                <p class="font-medium text-gray-700 text-theme-xs dark:text-gray-400">{{ t('guardians.relation') }}</p>
                <span class="flex flex-col gap-0.5">
                  <svg
                    class="fill-gray-300 dark:fill-gray-700"
                    width="8"
        ...
        <tbody>
          <tr
            v-for="guardian in paginatedData"
            :key="guardian.id"
            :class="{ 'bg-gray-50 dark:bg-gray-900': guardian.selected }"
          >
              ...
              >
                <span class="relative">
                  ...
                  <span
                    ...
                    class="flex h-4 w-4 items-center justify-center rounded-sm border-[1.25px]"
                  >
                    <span :class="guardian.selected ? '' : 'opacity-0'">
                      <svg
                        width="12"
                        height="12"
                        viewBox="0 0 12 12"
            ...
            <td class="px-4 py-3 border border-gray-100 dark:border-gray-800">
              <p class="text-gray-700 text-theme-sm dark:text-gray-400">{{ guardian.name }}</p>
            </td>
            <td class="px-4 py-3 border border-gray-100 dark:border-gray-800">
              <p class="text-gray-700 text-theme-sm dark:text-gray-400">{{ guardian.phone_number || '-' }}</p>
            </td>
            <td class="px-4 py-3 border border-gray-100 dark:border-gray-800">
              <p class="text-gray-700 text-theme-sm dark:text-gray-400">{{ guardian.relation || '-' }}</p>
            </td>
            <td class="px-4 py-3 border border-gray-100 dark:border-gray-800">
              <span
                class="inline-flex items-center justify-center gap-1 rounded-full px-2.5 py-0.5 text-xs font-medium"
                :class="{
                  'bg-green-50 text-green-600 dark:bg-green-500/15 dark:text-green-500': guardian.is_active,
                  'bg-error-50 text-error-600 dark:bg-error-500/15 dark:text-error-500': !guardian.is_active,
                }"
              >
                {{ guardian.is_active ? t('common.active') : t('common.inactive') }}
              </span>
            </td>
            <td class="px-4 py-3 border border-gray-100 dark:border-gray-800">
              <label :for="'toggle-' + guardian.id" class="cursor-pointer">
                <div class="relative">
                  <input
                    type="checkbox"
                    :id="'toggle-' + guardian.id"
                    class="sr-only"
                    :checked="guardian.is_active"
                    @change="toggleGuardianStatus(guardian)"
                  />
                  <div
                    class="block h-5 w-9 rounded-full"
                    :class="guardian.is_active ? 'bg-brand-500 dark:bg-brand-500' : 'bg-gray-200 dark:bg-white/10'"
                  ></div>
                  <div
                    :class="[
                      guardian.is_active
            ...
            <td class="px-4 py-3 border border-gray-100 dark:border-gray-800">
              <div class="flex items-center w-full gap-2">
                <Tooltip :text="t('messages.notAuthorized')" :show="!canView">
                  <button
                    :disabled="!canView"
                    @click="handleViewClick(guardian.id)"
                ...
                <Tooltip :text="t('messages.notAuthorized')" :show="!canEdit">
                  <button
                    :disabled="!canEdit"
                    @click="handleEditClick(guardian.id)"
                ...
                <Tooltip :text="t('messages.notAuthorized')" :show="!canDelete">
                  <button
                    :disabled="!canDelete"
                    @click="handleDeleteClick(guardian.id)"
  ...
  <DangerAlert
    ...
    :message="t('messages.deleteGuardianConfirmation')"
...
const canCreate = computed(() => hasAnyPermission(['guardians.create', 'guardians.store', 'guardians.add']))
const canView = computed(() => hasAnyPermission(['guardians.view', 'guardians.show', 'guardians.read']))
const canEdit = computed(() => hasAnyPermission(['guardians.update', 'guardians.edit']))
const canDelete = computed(() => hasAnyPermission(['guardians.delete', 'guardians.destroy']))

const { t } = useI18n()
const { success, error } = useNotifications()

const props = defineProps({ guardians: Object })

const search = ref('')
const sortColumn = ref('name')
const sortDirection = ref('asc')
const currentPage = ref(props.guardians?.current_page ?? 1)
const perPage = ref(props.guardians?.per_page ?? 10)
const selectAll = ref(false)

function goToCreate() {
  router.visit(route('admin.guardians.create'))
}

function goToView(id) {
  router.visit(route('admin.guardians.show', id))
}

function goToEdit(id) {
  router.visit(route('admin.guardians.edit', id))
}
...
const guardianToDeleteId = ref(null)

function openDeleteModal(id) {
  guardianToDeleteId.value = id
...
function closeDeleteModal() {
  ...
  guardianToDeleteId.value = null
}

function confirmDelete() {
  if (guardianToDeleteId.value) {
    router.delete(route('admin.guardians.destroy', guardianToDeleteId.value), {
      onSuccess: () => {
        success(t('guardians.guardianDeletedSuccessfully'))
        closeDeleteModal()
      },
      onError: () => {
        error(t('guardians.guardianDeletionFailed'))
        closeDeleteModal()
      },
      preserveScroll: true,
    })
...
function toggleGuardianStatus(guardian) {
  const wasActive = guardian.is_active
  guardian.is_active = !wasActive
  const url = wasActive
    ? route('admin.guardians.deactivate', { id: guardian.id })
    : route('admin.guardians.activate', { id: guardian.id })
  router.patch(url, {}, {
    preserveState: true,
    preserveScroll: true,
    onError: () => {
...
const filteredData = computed(() => {
  ...
  return (props.guardians?.data || [])
    .filter((g) => {
      const name = g.name?.toLowerCase() || ''
      const phone = (g.phone_number || '').toLowerCase()
      const relation = (g.relation || '').toLowerCase()
      return name.includes(searchLower) || phone.includes(searchLower) || relation.includes(searchLower)
    })
    .sort((a, b) => {
...
const totalEntries = computed(() => props.guardians?.total || filteredData.value.length)
const startEntry = computed(() => props.guardians?.from || 1)
const endEntry = computed(() => props.guardians?.to || filteredData.value.length)
const totalPages = computed(() => props.guardians?.last_page || 1)
...
const updateSelectAll = () => {
  ...
  selectAll.value = items.length > 0 && items.every((g) => g.selected)
}
