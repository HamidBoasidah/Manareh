<template>
      ...
      <table class="w-full min-w-full">
        <thead>
          <tr>
            ...
            <th class="px-4 py-3 text-start border border-gray-100 dark:border-gray-800">
              <div class="flex items-center justify-between w-full cursor-pointer" @click="sortBy('email')">
                <p class="font-medium text-gray-700 text-theme-xs dark:text-gray-400">{{ t('students.email') }}</p>
                <span class="flex flex-col gap-0.5">
                  <svg class="fill-gray-300 dark:fill-gray-700" width="8" height="5" viewBox="0 0 8 5" fill="none">
                    <path d="M4.40962 0.585167C4.21057 0.300808 3.78943 0.300807 3.59038 0.585166L1.05071 4.21327C0.81874 4.54466 1.05582 5 1.46033 5H6.53967C6.94418 5 7.18126 4.54466 6.94929 4.21327L4.40962 0.585167Z" />
                  </svg>
            ...
            <th class="px-4 py-3 text-start border border-gray-100 dark:border-gray-800">
              <div class="flex items-center justify-between w-full cursor-pointer" @click="sortBy('guardian_name')">
                <p class="font-medium text-gray-700 text-theme-xs dark:text-gray-400">{{ t('students.guardian') }}</p>
                <span class="flex flex-col gap-0.5">
                  <svg class="fill-gray-300 dark:fill-gray-700" width="8" height="5" viewBox="0 0 8 5" fill="none">
                    <path d="M4.40962 0.585167C4.21057 0.300808 3.78943 0.300807 3.59038 0.585166L1.05071 4.21327C0.81874 4.54466 1.05582 5 1.46033 5H6.53967C6.94418 5 7.18126 4.54466 6.94929 4.21327L4.40962 0.585167Z" />
                  </svg>
        ...
        <tbody>
          <tr
            ...
            :key="student.id"
            :class="{ 'bg-gray-50 dark:bg-gray-900': student.selected }"
          >
            <td class="px-4 py-3 border border-gray-100 dark:border-gray-800">
              <label class="flex items-center text-sm font-medium text-gray-700 cursor-pointer select-none dark:text-gray-400">
                <span class="relative">
                  <input type="checkbox" class="sr-only" v-model="student.selected" @change="updateSelectAll" />
                  <span
                    :class="student.selected ? 'border-brand-500 bg-brand-500' : 'bg-transparent border-gray-300 dark:border-gray-700'"
                    class="flex h-4 w-4 items-center justify-center rounded-sm border-[1.25px]"
                  >
                    <span :class="student.selected ? '' : 'opacity-0'">
                      <svg width="12" height="12" viewBox="0 0 12 12" fill="none">
                        <path d="M10 3L4.5 8.5L2 6" stroke="white" stroke-width="1.6666" stroke-linecap="round" stroke-linejoin="round" />
                      </svg>
                    </span>
            ...
            <td class="px-4 py-3 border border-gray-100 dark:border-gray-800">
              <p class="text-gray-700 text-theme-sm dark:text-gray-400">{{ student.user_name ?? '—' }}</p>
            </td>

            <td class="px-4 py-3 border border-gray-100 dark:border-gray-800">
              <p class="text-gray-700 text-theme-sm dark:text-gray-400">{{ student.email ?? '—' }}</p>
            </td>
            ...
            <td class="px-4 py-3 border border-gray-100 dark:border-gray-800">
              <label :for="'toggle-' + student.id" class="cursor-pointer">
                <div class="relative">
                  <input
                    type="checkbox"
                  ...
                  />
                  <div class="block h-5 w-9 rounded-full" :class="student.is_active ? 'bg-brand-500 dark:bg-brand-500' : 'bg-gray-200 dark:bg-white/10'"></div>
                  <div :class="[student.is_active ? 'rtl:translate-x-[-100%] ltr:translate-x-full' : 'translate-x-0']" class="shadow-theme-sm absolute top-0.5 h-4 w-4 rounded-full bg-white duration-200 ease-linear rtl:right-0.5 ltr:left-0.5"></div>
                </div>
              </label>
            </td>
            ...
            <td class="px-4 py-3 border border-gray-100 dark:border-gray-800">
              <div class="flex items-center w-full gap-2">
                <Tooltip :text="t('messages.notAuthorized')" :show="!canView">
                  <button
                    :disabled="!canView"
                    @click="handleViewClick(student.id)"
  ...
  <DangerAlert
    ...
    :message="t('messages.deleteStudentConfirmation')"
...
const canCreate = computed(() => hasAnyPermission(['students.create', 'students.store', 'students.add']))
const canView = computed(() => hasAnyPermission(['students.view', 'students.show', 'students.read']))
const canEdit = computed(() => hasAnyPermission(['students.update', 'students.edit']))
const canDelete = computed(() => hasAnyPermission(['students.delete', 'students.destroy']))
...
function goToCreate() { router.visit(route('admin.students.create')) }
function goToView(id) { router.visit(route('admin.students.show', id)) }
function goToEdit(id) { router.visit(route('admin.students.edit', id)) }
...
function openDeleteModal(id) { studentToDeleteId.value = id; isDeleteModalOpen.value = true }
function handleDeleteClick(id) { if (!canDelete.value) return; openDeleteModal(id) }
function closeDeleteModal() { isDeleteModalOpen.value = false; studentToDeleteId.value = null }

function confirmDelete() {
  if (studentToDeleteId.value) {
    router.delete(route('admin.students.destroy', studentToDeleteId.value), {
      onSuccess: () => {
        success(t('students.studentDeletedSuccessfully'))
        closeDeleteModal()
      },
      onError: () => {
        error(t('students.studentDeletionFailed'))
        closeDeleteModal()
      },
      preserveScroll: true,
    })
...
function toggleStudentStatus(student) {
  const previous = student.is_active
  student.is_active = !previous
  const url = previous
    ? route('admin.students.deactivate', { id: student.id })
    : route('admin.students.activate', { id: student.id })
  router.patch(url, {}, {
    preserveState: true,
    preserveScroll: true,
    onError: () => {
...
const filteredData = computed(() => {
  ...
  const items = (props.students?.data || []).filter((student) => {
    const name = student.user_name?.toLowerCase() || ''
    const email = student.email?.toLowerCase() || ''
    const guardian = student.guardian_name?.toLowerCase() || ''
    const mosque = student.mosque_name?.toLowerCase() || ''
    const phone = student.phone_number?.toLowerCase() || ''
    const whatsapp = student.whatsapp_number?.toLowerCase() || ''
    const address = student.address?.toLowerCase() || ''
    const nationality = student.nationality?.toLowerCase() || ''
    return (
      name.includes(term) ||
      email.includes(term) ||
      guardian.includes(term) ||
      mosque.includes(term) ||
      phone.includes(term) ||
...
const totalEntries = computed(() => props.students?.total || filteredData.value.length)
const startEntry = computed(() => props.students?.from || (filteredData.value.length ? 1 : 0))
const endEntry = computed(() => props.students?.to || filteredData.value.length)
const totalPages = computed(() => props.students?.last_page || 1)
...
watch(() => props.students?.current_page, (val) => {
  currentPage.value = typeof val === 'number' ? val : 1
})

watch(() => props.students?.per_page, (val) => {
...
const toggleSelectAll = () => {
  filteredData.value.forEach((student) => {
...
const updateSelectAll = () => {
  ...
  selectAll.value = items.length > 0 && items.every((student) => student.selected)
}
