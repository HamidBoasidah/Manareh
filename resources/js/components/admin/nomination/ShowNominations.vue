  ...
  <div class="overflow-hidden">
    ...
    <div class="max-w-full overflow-x-auto">
      <table class="w-full min-w-full">
          ...
          <tr>
            ...
            <th class="px-4 py-3 text-start border border-gray-100 dark:border-gray-800">
              <div class="flex items-center justify-between w-full cursor-pointer" @click="sortBy('student_name')">
                <p class="font-medium text-gray-700 text-theme-xs dark:text-gray-400">{{ t('daily_studies.student') }}</p>
              </div>
            </th>
            ...
            <th class="px-4 py-3 text-start border border-gray-100 dark:border-gray-800">
              <div class="flex items-center justify-between w-full cursor-pointer" @click="sortBy('nomination_type')">
                <p class="font-medium text-gray-700 text-theme-xs dark:text-gray-400">{{ t('nominations.type') }}</p>
              </div>
            </th>

            <!-- plan column removed as requested -->
        ...
        <tbody>
          <tr
            v-for="(n, idx) in paginatedData"
            :key="n.id"
            :class="{ 'bg-gray-50 dark:bg-gray-900': n.selected }"
          >
            <td class="px-4 py-3 border border-gray-100 dark:border-gray-800">
              <label class="flex items-center text-sm font-medium text-gray-700 cursor-pointer select-none dark:text-gray-400">
                <span class="relative">
                  <input type="checkbox" class="sr-only" v-model="n.selected" @change="updateSelectAll" />
                  <span
                    :class="n.selected ? 'border-brand-500 bg-brand-500' : 'bg-transparent border-gray-300 dark:border-gray-700'"
                    class="flex h-4 w-4 items-center justify-center rounded-sm border-[1.25px]"
                  >
                    <span :class="n.selected ? '' : 'opacity-0'">
                      <svg width="12" height="12" viewBox="0 0 12 12" fill="none">
                        <path d="M10 3L4.5 8.5L2 6" stroke="white" stroke-width="1.6666" stroke-linecap="round" stroke-linejoin="round"/>
                      </svg>
                    </span>
            ...
            <td class="px-4 py-3 border border-gray-100 dark:border-gray-800">
              <p class="text-gray-700 text-theme-sm dark:text-gray-400">{{ n.student_name ?? '—' }}</p>
            </td>

            <td class="px-4 py-3 border border-gray-100 dark:border-gray-800">
              <p class="text-gray-700 text-theme-sm dark:text-gray-400">{{ n.circle_name ?? '—' }}</p>
            </td>

            <td class="px-4 py-3 border border-gray-100 dark:border-gray-800">
              <p class="text-gray-700 text-theme-sm dark:text-gray-400">{{ t('nominations.types.' + n.nomination_type) || n.nomination_type }}</p>
            </td>
            ...
            <td class="px-4 py-3 border border-gray-100 dark:border-gray-800">
              <span
                ...
                :class="{
                  'bg-green-50 text-green-600 dark:bg-green-500/15 dark:text-green-500': n.is_active,
                  'bg-error-50 text-error-600 dark:bg-error-500/15 dark:text-error-500': !n.is_active,
                }"
              >
            ...
            <td class="px-4 py-3 border border-gray-100 dark:border-gray-800">
              <label :for="'toggle-' + n.id" class="cursor-pointer">
                <div class="relative">
                  <input
                    type="checkbox"
                    :id="'toggle-' + n.id"
                    class="sr-only"
                    :checked="n.is_active"
                    @change="toggleNominationStatus(n)
                    "
                  />
                  <div class="block h-5 w-9 rounded-full" :class="n.is_active ? 'bg-brand-500 dark:bg-brand-500' : 'bg-gray-200 dark:bg-white/10'"></div>
                  <div :class="[n.is_active ? 'rtl:translate-x-[-100%] ltr:translate-x-full' : 'translate-x-0']" class="shadow-theme-sm absolute top-0.5 h-4 w-4 rounded-full bg-white duration-200 ease-linear rtl:right-0.5 ltr:left-0.5"></div>
                </div>
              </label>
            </td>
          ...
          <tr v-if="paginatedData.length === 0">
            <td colspan="7" class="px-4 py-6 text-center text-sm text-gray-500 dark:text-gray-400">
              {{ t('nominations.noNominations') || 'No nominations found' }}
            </td>
          </tr>
        </tbody>
      </table>
    ...
    <DangerAlert
      ...
      :message="t('messages.deleteNominationConfirmation')"
...
const canCreate = computed(() => hasAnyPermission(['nominations.create', 'nominations.store', 'nominations.add']))
const canView   = computed(() => hasAnyPermission(['nominations.view', 'nominations.show', 'nominations.read']))
const canEdit   = computed(() => hasAnyPermission(['nominations.update', 'nominations.edit']))
const canDelete = computed(() => hasAnyPermission(['nominations.delete', 'nominations.destroy']))

const { t } = useI18n()
const { success, error } = useNotifications()

const props = defineProps({ nominations: Object })

const search         = ref('')
const sortColumn     = ref('student_name')
const sortDirection  = ref('asc')
const currentPage    = ref(props.nominations?.current_page ?? 1)
const perPage        = ref(props.nominations?.per_page ?? 10)
const selectAll      = ref(false)

function goToCreate()      { router.visit(route('admin.nominations.create')) }
function goToView(id)      { router.visit(route('admin.nominations.show', id)) }
function goToEdit(id)      { router.visit(route('admin.nominations.edit', id)) }
...
const nominationToDeleteId = ref(null)

function openDeleteModal(id)  { nominationToDeleteId.value = id; isDeleteModalOpen.value = true }
function handleDeleteClick(id){ if (!canDelete.value) return; openDeleteModal(id) }
function closeDeleteModal()   { isDeleteModalOpen.value = false; nominationToDeleteId.value = null }

function confirmDelete() {
  if (nominationToDeleteId.value) {
    router.delete(route('admin.nominations.destroy', nominationToDeleteId.value), {
      onSuccess: () => { success(t('nominations.nominationDeletedSuccessfully')); closeDeleteModal() },
      onError:   () => { error(t('nominations.nominationDeletionFailed'));       closeDeleteModal() },
      preserveScroll: true,
    })
  }
}

function toggleNominationStatus(n) {
  const wasActive = n.is_active
  n.is_active = !wasActive
  const url = wasActive
    ? route('admin.nominations.deactivate', { id: n.id })
    : route('admin.nominations.activate',   { id: n.id })
  router.patch(url, {}, {
    preserveState: true,
    preserveScroll: true,
    onError: () => { n.is_active = wasActive },
  })
}

const filteredData = computed(() => {
  const searchLower = (search.value || '').toLowerCase()
  return (props.nominations?.data || [])
    .filter((d) => {
      const student = (d.student_name || '').toLowerCase()
      const circle  = (d.circle_name || '').toLowerCase()
      const plan    = (d.academic_year_name || '').toLowerCase()
      return student.includes(searchLower) || circle.includes(searchLower) || plan.includes(searchLower)
    })
    .sort((a, b) => {
...
const totalEntries = computed(() => props.nominations?.total || filteredData.value.length)
const startEntry   = computed(() => props.nominations?.from  || 1)
const endEntry     = computed(() => props.nominations?.to    || filteredData.value.length)
const totalPages   = computed(() => props.nominations?.last_page || 1)
...
watch(() => props.nominations?.current_page, (val) => {
  currentPage.value = typeof val === 'number' ? val : 1
})
watch(() => props.nominations?.per_page, (val) => {
