...
const props = defineProps({
  districts: { type: Array, required: true }, // [{id, name_ar, name_en}]
})
...
function create() {
  form.post(route('admin.areas.store'), {
    onSuccess: () => {
      success(t('areas.areaCreatedSuccessfully'))
      // controller may redirect; toast will remain
    },
    onError: () => {
      error(t('areas.areaCreationFailed'))
    },
    preserveScroll: true,
  })
}
