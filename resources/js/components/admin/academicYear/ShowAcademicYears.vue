<template>
  <div class="overflow-hidden">
    ...
    <div class="max-w-full overflow-x-auto">
      <table class="w-full min-w-full">
          ...
          <tr>
            ...
            <th class="px-4 py-3 text-start border border-gray-100 dark:border-gray-800">
              <div class="flex items-center justify-between w-full cursor-pointer" @click="sortBy('name')">
                <p class="font-medium text-gray-700 text-theme-xs dark:text-gray-400">{{ t('academic_years.academic_year') }}</p>
            ...
            <th class="px-4 py-3 text-start border border-gray-100 dark:border-gray-800">
              <div class="flex items-center justify-between w-full cursor-pointer" @click="sortBy('mosque_name')">
                <p class="font-medium text-gray-700 text-theme-xs dark:text-gray-400">{{ t('academicYears.mosqueName') }}</p>
                <span class="flex flex-col gap-0.5">
                  <svg class="fill-gray-300 dark:fill-gray-700" width="8" height="5" viewBox="0 0 8 5" fill="none"><path d="M4.40962 0.585167C4.21057 0.300808 3.78943 0.300807 3.59038 0.585166L1.05071 4.21327C0.81874 4.54466 1.05582 5 1.46033 5H6.53967C6.94418 5 7.18126 4.54466 6.94929 4.21327L4.40962 0.585167Z"/></svg>
                  <svg class="fill-gray-300 dark:fill-gray-700" width="8" height="5" viewBox="0 0 8 5" fill="none"><path d="M4.40962 4.41483C4.21057 4.69919 3.78943 4.69919 3.59038 4.41483L1.05071 0.786732C0.81874 0.455343 1.05582 0 1.46033 0H6.53967C6.94418 0 7.18126 0 6.94929 0.786731L4.40962 4.41483Z"/></svg>
                </span>
              </div>
            </th>

            <th class="px-4 py-3 text-start border border-gray-100 dark:border-gray-800">
              <div class="flex items-center justify-between w-full cursor-pointer" @click="sortBy('start_date_g')">
                <p class="font-medium text-gray-700 text-theme-xs dark:text-gray-400">{{ t('academic_years.startDate') }}</p>
                <span class="flex flex-col gap-0.5">
                  <svg class="fill-gray-300 dark:fill-gray-700" width="8" height="5" viewBox="0 0 8 5" fill="none"><path d="M4.40962 0.585167C4.21057 0.300808 3.78943 0.300807 3.59038 0.585166L1.05071 4.21327C0.81874 4.54466 1.05582 5 1.46033 5H6.53967C6.94418 5 7.18126 4.54466 6.94929 4.21327L4.40962 0.585167Z"/></svg>
                  <svg class="fill-gray-300 dark:fill-gray-700" width="8" height="5" viewBox="0 0 8 5" fill="none"><path d="M4.40962 4.41483C4.21057 4.69919 3.78943 4.69919 3.59038 4.41483L1.05071 0.786732C0.81874 0.455343 1.05582 0 1.46033 0H6.53967C6.94418 0 7.18126 0 6.94929 0.786731L4.40962 4.41483Z"/></svg>
                </span>
            ...
            <th class="px-4 py-3 text-start border border-gray-100 dark:border-gray-800">
              <div class="flex items-center justify-between w-full cursor-pointer" @click="sortBy('end_date_g')">
                <p class="font-medium text-gray-700 text-theme-xs dark:text-gray-400">{{ t('academic_years.endDate') }}</p>
                <span class="flex flex-col gap-0.5">
                  <svg class="fill-gray-300 dark:fill-gray-700" width="8" height="5" viewBox="0 0 8 5" fill="none"><path d="M4.40962 0.585167C4.21057 0.300808 3.78943 0.300807 3.59038 0.585166L1.05071 4.21327C0.81874 4.54466 1.05582 5 1.46033 5H6.53967C6.94418 5 7.18126 4.54466 6.94929 4.21327L4.40962 0.585167Z"/></svg>
                  <svg class="fill-gray-300 dark:fill-gray-700" width="8" height="5" viewBox="0 0 8 5" fill="none"><path d="M4.40962 4.41483C4.21057 4.69919 3.78943 4.69919 3.59038 4.41483L1.05071 0.786732C0.81874 0.455343 1.05582 0 1.46033 0H6.53967C6.94418 0 7.18126 0 6.94929 0.786731L4.40962 4.41483Z"/></svg>
                </span>
        ...
        <tbody>
          <tr
            v-for="ay in paginatedData"
            :key="ay.id"
            :class="{ 'bg-gray-50 dark:bg-gray-900': ay.selected }"
          >
            <td class="px-4 py-3 border border-gray-100 dark:border-gray-800">
              <label class="flex items-center text-sm font-medium text-gray-700 cursor-pointer select-none dark:text-gray-400">
                <span class="relative">
                  <input type="checkbox" class="sr-only" v-model="ay.selected" @change="updateSelectAll" />
                  <span :class="ay.selected ? 'border-brand-500 bg-brand-500' : 'bg-transparent border-gray-300 dark:border-gray-700'" class="flex h-4 w-4 items-center justify-center rounded-sm border-[1.25px]">
                    <span :class="ay.selected ? '' : 'opacity-0'">
            ...
            <td class="px-4 py-3 border border-gray-100 dark:border-gray-800">
              <p class="text-gray-700 text-theme-sm dark:text-gray-400">{{ ay.name }}</p>
            </td>

            <td class="px-4 py-3 border border-gray-100 dark:border-gray-800">
              <p class="text-gray-700 text-theme-sm dark:text-gray-400">{{ ay.mosque_name }}</p>
            </td>

            <td class="px-4 py-3 border border-gray-100 dark:border-gray-800">
              <p class="text-gray-700 text-theme-sm dark:text-gray-400">{{ formatDate(ay.start_date_g) }}</p>
            </td>

            <td class="px-4 py-3 border border-gray-100 dark:border-gray-800">
              <p class="text-gray-700 text-theme-sm dark:text-gray-400">{{ formatDate(ay.end_date_g) }}</p>
            </td>
            ...
            <td class="px-4 py-3 border border-gray-100 dark:border-gray-800">
              <span
                ...
                :class="{
                  'bg-green-50 text-green-600 dark:bg-green-500/15 dark:text-green-500': ay.is_active,
                  'bg-error-50 text-error-600 dark:bg-error-500/15 dark:text-error-500': !ay.is_active,
                }"
              >
                {{ ay.is_active ? t('common.active') : t('common.inactive') }}
              </span>
            </td>

            <td class="px-4 py-3 border border-gray-100 dark:border-gray-800">
              <label :for="'toggle-' + ay.id" class="cursor-pointer">
                <div class="relative">
                  <input
                    type="checkbox"
                    :id="'toggle-' + ay.id"
                    ...
                    @change="toggleAcademicYearStatus(ay)"
                  />
                  <div class="block h-5 w-9 rounded-full" :class="ay.is_active ? 'bg-brand-500 dark:bg-brand-500' : 'bg-gray-200 dark:bg-white/10'"></div>
                  <div :class="[ay.is_active ? 'rtl:translate-x-[-100%] ltr:translate-x-full' : 'translate-x-0']" class="shadow-theme-sm absolute top-0.5 h-4 w-4 rounded-full bg-white duration-200 ease-linear rtl:right-0.5 ltr:left-0.5"></div>
                </div>
              </label>
            </td>
            ...
            <td class="px-4 py-3 border border-gray-100 dark:border-gray-800">
              <div class="flex items-center w-full gap-2">
                <Tooltip :text="t('messages.notAuthorized')" :show="!canView">
                  <button :disabled="!canView" @click="handleViewClick(ay.id)" class="text-gray-500 hover:text-gray-800 dark:text-gray-400 dark:hover:text-white/90 disabled:text-gray-400 disabled:dark:text-gray-500">
                    <svg class="fill-current" width="21" height="20" viewBox="0 0 21 20" fill="none"><path fill-rule="evenodd" clip-rule="evenodd" d="M10.8749 13.8619C8.10837 13.8619 5.74279 12.1372 4.79804 9.70241C5.74279 7.26761 8.10837 5.54297 10.8749 5.54297C13.6415 5.54297 16.0071 7.26762 16.9518 9.70243C16.0071 12.1372 13.6415 13.8619 10.8749 13.8619ZM10.8749 4.04297C7.35666 4.04297 4.36964 6.30917 3.29025 9.4593C3.23626 9.61687 3.23626 9.78794 3.29025 9.94552C4.36964 13.0957 7.35666 15.3619 10.8749 15.3619C14.3932 15.3619 17.3802 13.0957 18.4596 9.94555C18.5136 9.78797 18.5136 9.6169 18.4596 9.45932C17.3802 6.30919 14.3932 4.04297 10.8749 4.04297ZM10.8663 7.84413C9.84002 7.84413 9.00808 8.67606 9.00808 9.70231C9.00808 10.7286 9.84002 11.5605 10.8663 11.5605H10.8811C11.9074 11.5605 12.7393 10.7286 12.7393 9.70231C12.7393 8.67606 11.9074 7.84413 10.8811 7.84413H10.8663Z"/></svg>
                  </button>
                </Tooltip>

                <Tooltip :text="t('messages.notAuthorized')" :show="!canEdit">
                  <button :disabled="!canEdit" @click="handleEditClick(ay.id)" class="text-gray-500 hover:text-blue-500 dark:text-gray-400 dark:hover:text-blue-300 disabled:text-gray-400 disabled:dark:text-gray-500">
                ...
                <Tooltip :text="t('messages.notAuthorized')" :show="!canDelete">
                  <button :disabled="!canDelete" @click="handleDeleteClick(ay.id)" class="text-gray-500 hover:text-error-500 dark:text-gray-400 dark:hover:text-error-500 disabled:text-gray-400 disabled:dark:text-gray-500">
          ...
          <tr v-if="paginatedData.length === 0">
            <td colspan="8" class="px-4 py-6 text-center text-sm text-gray-500 dark:text-gray-400">
              {{ t('academicYears.noAcademicYear') }}
            </td>
          </tr>
        </tbody>
      </table>
  ...
  <DangerAlert
    ...
    :message="t('messages.deleteAcademicYearConfirmation')"
...
const canCreate = computed(() => hasAnyPermission(['academic_years.create', 'academic_years.store', 'academic_years.add']))
const canView   = computed(() => hasAnyPermission(['academic_years.view', 'academic_years.show', 'academic_years.read']))
const canEdit   = computed(() => hasAnyPermission(['academic_years.update', 'academic_years.edit']))
const canDelete = computed(() => hasAnyPermission(['academic_years.delete', 'academic_years.destroy']))
...
const currentPage    = ref(props.academicYears?.current_page ?? 1)
const perPage        = ref(props.academicYears?.per_page ?? 10)
const selectAll      = ref(false)

function goToCreate()      { router.visit(route('admin.academic_years.create')) }
function goToView(id)      { router.visit(route('admin.academic_years.show', id)) }
function goToEdit(id)      { router.visit(route('admin.academic_years.edit', id)) }
...
const academicYearToDeleteId = ref(null)

function openDeleteModal(id)  { academicYearToDeleteId.value = id; isDeleteModalOpen.value = true }
function handleDeleteClick(id){ if (!canDelete.value) return; openDeleteModal(id) }
function closeDeleteModal()   { isDeleteModalOpen.value = false; academicYearToDeleteId.value = null }

function confirmDelete() {
  if (academicYearToDeleteId.value) {
    router.delete(route('admin.academic_years.destroy', academicYearToDeleteId.value), {
      onSuccess: () => { success(t('academicYears.academicYearDeletedSuccessfully')); closeDeleteModal() },
      onError:   () => { error(t('academicYears.academicYearDeletionFailed'));       closeDeleteModal() },
      preserveScroll: true,
    })
  }
}

function toggleAcademicYearStatus(ay) {
  const wasActive = ay.is_active
  ay.is_active = !wasActive
  const url = wasActive
    ? route('admin.academic_years.deactivate', { id: ay.id })
    : route('admin.academic_years.activate',   { id: ay.id })
  router.patch(url, {}, {
    preserveState: true,
    preserveScroll: true,
    onError: () => { ay.is_active = wasActive },
  })
}

const filteredData = computed(() => {
  const searchLower = (search.value || '').toLowerCase()
  return (props.academicYears?.data || [])
    .filter((d) => {
      const name = d.name?.toLowerCase() || ''
      const mosque = d.mosque_name?.toLowerCase() || ''
      return name.includes(searchLower) || mosque.includes(searchLower)
    })
    .sort((a, b) => {
...
const totalEntries = computed(() => props.academicYears?.total || filteredData.value.length)
const startEntry = computed(() => props.academicYears?.from || 1)
const endEntry = computed(() => props.academicYears?.to || filteredData.value.length)
const totalPages = computed(() => props.academicYears?.last_page || 1)
...
watch(
  () => props.academicYears?.current_page,
  (val) => {
