      ...
      <div class="p-4 sm:p-6 dark:border-gray-800">
        <div class="grid grid-cols-1 gap-5 md:grid-cols-2">
          <div class="md:col-span-2">
            <label for="circle-classification-name" class="mb-1.5 block text-sm font-medium text-gray-700 dark:text-gray-400">
              {{ t('common.name') }}
            </label>
            <input
              v-model="form.name"
...
function create() {
  form.post(route('admin.circle_classifications.store'), {
    onSuccess: () => {
      success(t('circle_classifications.circleClassificationCreatedSuccessfully'))
      form.reset()
    },
    onError: () => {
      error(t('circle_classifications.circleClassificationCreationFailed'))
    },
    preserveScroll: true,
  })
}
