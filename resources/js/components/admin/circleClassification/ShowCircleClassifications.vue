<template>
  <div class="overflow-hidden">
      ...
      <div class="flex flex-col gap-3 sm:flex-row sm:items-center">
        ...
        <Tooltip :text="t('messages.notAuthorized')" :show="!canCreate">
          ...
          >
            ...
            {{ t('circle_classifications.addCircleClassification') }}
          </button>
        </Tooltip>
      </div>
    </div>
    ...
    <div class="max-w-full overflow-x-auto">
      <table class="w-full min-w-full">
        ...
        <tbody>
          <tr
            v-for="classification in paginatedData"
            :key="classification.id"
            :class="{ 'bg-gray-50 dark:bg-gray-900': classification.selected }"
          >
              ...
              >
                <span class="relative">
                  ...
                  <span
                    ...
                    class="flex h-4 w-4 items-center justify-center rounded-sm border-[1.25px]"
                  >
                    <span :class="classification.selected ? '' : 'opacity-0'">
                      <svg
                        width="12"
                        height="12"
                        viewBox="0 0 12 12"
            ...
            <td class="px-4 py-3 border border-gray-100 dark:border-gray-800">
              <p class="text-gray-700 text-theme-sm dark:text-gray-400">{{ classification.name }}</p>
            </td>
            <td class="px-4 py-3 border border-gray-100 dark:border-gray-800">
              <span
                class="inline-flex items-center justify-center gap-1 rounded-full px-2.5 py-0.5 text-xs font-medium"
                :class="{
                  'bg-green-50 text-green-600 dark:bg-green-500/15 dark:text-green-500': classification.is_active,
                  'bg-error-50 text-error-600 dark:bg-error-500/15 dark:text-error-500': !classification.is_active,
                }"
              >
                {{ classification.is_active ? t('common.active') : t('common.inactive') }}
              </span>
            </td>
            <td class="px-4 py-3 border border-gray-100 dark:border-gray-800">
              <label :for="'toggle-' + classification.id" class="cursor-pointer">
                <div class="relative">
                  <input
                    type="checkbox"
                    :id="'toggle-' + classification.id"
                    class="sr-only"
                    :checked="classification.is_active"
                    @change="toggleClassificationStatus(classification)"
                  />
                  <div
                    class="block h-5 w-9 rounded-full"
                    :class="classification.is_active ? 'bg-brand-500 dark:bg-brand-500' : 'bg-gray-200 dark:bg-white/10'"
                  ></div>
                  <div
                    :class="[
                      classification.is_active
                        ? 'rtl:translate-x-[-100%] ltr:translate-x-full'
                        : 'translate-x-0',
                    ]"
            ...
            <td class="px-4 py-3 border border-gray-100 dark:border-gray-800">
              <div class="flex items-center w-full gap-2">
                <Tooltip :text="t('messages.notAuthorized')" :show="!canView">
                  <button
                    :disabled="!canView"
                    @click="handleViewClick(classification.id)"
                ...
                <Tooltip :text="t('messages.notAuthorized')" :show="!canEdit">
                  <button
                    :disabled="!canEdit"
                    @click="handleEditClick(classification.id)"
                ...
                <Tooltip :text="t('messages.notAuthorized')" :show="!canDelete">
                  <button
                    :disabled="!canDelete"
                    @click="handleDeleteClick(classification.id)"
          ...
          <tr v-if="paginatedData.length === 0">
            <td colspan="5" class="px-4 py-6 text-center text-sm text-gray-500 dark:text-gray-400">
              {{ t('circle_classifications.noCircleClassification') }}
            </td>
          </tr>
        </tbody>
      </table>
  ...
  <DangerAlert
    ...
    :message="t('messages.deleteCircleClassificationConfirmation')"
...
const canCreate = computed(() => hasAnyPermission(['circle_classifications.create', 'circle_classifications.store', 'circle_classifications.add']))
const canView = computed(() => hasAnyPermission(['circle_classifications.view', 'circle_classifications.show', 'circle_classifications.read']))
const canEdit = computed(() => hasAnyPermission(['circle_classifications.update', 'circle_classifications.edit']))
const canDelete = computed(() => hasAnyPermission(['circle_classifications.delete', 'circle_classifications.destroy']))

const { t } = useI18n()
const { success, error } = useNotifications()

const props = defineProps({ classifications: Object })

const search = ref('')
const sortColumn = ref('name')
const sortDirection = ref('asc')
const currentPage = ref(props.classifications?.current_page ?? 1)
const perPage = ref(props.classifications?.per_page ?? 10)
const selectAll = ref(false)

function goToCreate() {
  router.visit(route('admin.circle_classifications.create'))
}

function goToView(id) {
  router.visit(route('admin.circle_classifications.show', id))
}

function goToEdit(id) {
  router.visit(route('admin.circle_classifications.edit', id))
}
...
const classificationToDeleteId = ref(null)

function openDeleteModal(id) {
  classificationToDeleteId.value = id
...
function closeDeleteModal() {
  ...
  classificationToDeleteId.value = null
}

function confirmDelete() {
  if (classificationToDeleteId.value) {
    router.delete(route('admin.circle_classifications.destroy', classificationToDeleteId.value), {
      onSuccess: () => {
        success(t('circle_classifications.circleClassificationDeletedSuccessfully'))
        closeDeleteModal()
      },
      onError: () => {
        error(t('circle_classifications.circleClassificationDeletionFailed'))
        closeDeleteModal()
      },
      preserveScroll: true,
    })
...
function toggleClassificationStatus(classification) {
  const wasActive = classification.is_active
  classification.is_active = !wasActive
  const url = wasActive
    ? route('admin.circle_classifications.deactivate', { id: classification.id })
    : route('admin.circle_classifications.activate', { id: classification.id })
  router.patch(url, {}, {
    preserveState: true,
    preserveScroll: true,
    onError: () => {
...
const totalEntries = computed(() => props.classifications?.total || filteredData.value.length)
const startEntry = computed(() => props.classifications?.from || 1)
const endEntry = computed(() => props.classifications?.to || filteredData.value.length)
const totalPages = computed(() => props.classifications?.last_page || 1)
...
watch(
  () => props.classifications?.current_page,
  (val) => {
...
watch(
  () => props.classifications?.per_page,
  (val) => {
...
const toggleSelectAll = () => {
  filteredData.value.forEach((classification) => {
    classification.selected = selectAll.value
  })
}

const updateSelectAll = () => {
  const items = filteredData.value
  selectAll.value = items.length > 0 && items.every((classification) => classification.selected)
}
