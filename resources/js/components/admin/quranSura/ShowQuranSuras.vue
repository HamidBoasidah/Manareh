<template>
      ...
      <table class="w-full min-w-full">
        <thead>
          <tr>
            ...
            <th class="px-4 py-3 text-start border border-gray-100 dark:border-gray-800 w-24">
              <div class="flex items-center justify-between w-full cursor-pointer" @click="sortBy('id')">
                <p class="font-medium text-gray-700 text-theme-xs dark:text-gray-400">{{ t('quran_suras.number') }}</p>
                <span class="flex flex-col gap-0.5">
                  <svg
                    :class="getSortIconClasses('id', 'asc')"
                    width="8"
                    height="5"
                    viewBox="0 0 8 5"
                    fill="none"
                  ...
                  <svg
                    :class="getSortIconClasses('id', 'desc')"
                    width="8"
                    height="5"
                    viewBox="0 0 8 5"
                    fill="none"
            ...
            <th class="px-4 py-3 text-start border border-gray-100 dark:border-gray-800">
              <div class="flex items-center justify-between w-full cursor-pointer" @click="sortBy('name_ar')">
                <p class="font-medium text-gray-700 text-theme-xs dark:text-gray-400">{{ t('quran_suras.nameAr') }}</p>
                <span class="flex flex-col gap-0.5">
                  <svg
                    :class="getSortIconClasses('name_ar', 'asc')"
                    width="8"
                    height="5"
                    viewBox="0 0 8 5"
                    fill="none"
                  ...
                  <svg
                    :class="getSortIconClasses('name_ar', 'desc')"
                    width="8"
                    height="5"
                    viewBox="0 0 8 5"
                    fill="none"
            ...
            <th class="px-4 py-3 text-start border border-gray-100 dark:border-gray-800">
              <div class="flex items-center justify-between w-full cursor-pointer" @click="sortBy('name_en')">
                <p class="font-medium text-gray-700 text-theme-xs dark:text-gray-400">{{ t('quran_suras.nameEn') }}</p>
                <span class="flex flex-col gap-0.5">
                  <svg
                    :class="getSortIconClasses('name_en', 'asc')"
                    width="8"
                    height="5"
                    viewBox="0 0 8 5"
                    fill="none"
                  ...
                  <svg
                    :class="getSortIconClasses('name_en', 'desc')"
                    width="8"
                    height="5"
                    viewBox="0 0 8 5"
                    fill="none"
            ...
            <th class="px-4 py-3 text-start border border-gray-100 dark:border-gray-800">
              <div class="flex items-center justify-between w-full cursor-pointer" @click="sortBy('ayah_count')">
                <p class="font-medium text-gray-700 text-theme-xs dark:text-gray-400">{{ t('quran_suras.ayahCount') }}</p>
                <span class="flex flex-col gap-0.5">
                  <svg
                    :class="getSortIconClasses('ayah_count', 'asc')"
                    width="8"
                    height="5"
                    viewBox="0 0 8 5"
                    fill="none"
                  ...
                  <svg
                    :class="getSortIconClasses('ayah_count', 'desc')"
                    width="8"
                    height="5"
                    viewBox="0 0 8 5"
                    fill="none"
            ...
            <th class="px-4 py-3 text-start border border-gray-100 dark:border-gray-800">
              <div class="flex items-center justify-between w-full cursor-pointer" @click="sortBy('is_active')">
                ...
                <span class="flex flex-col gap-0.5">
                  <svg
                    :class="getSortIconClasses('is_active', 'asc')"
                    width="8"
                    height="5"
                    viewBox="0 0 8 5"
                    fill="none"
                  ...
                  <svg
                    :class="getSortIconClasses('is_active', 'desc')"
                    width="8"
                    height="5"
                    viewBox="0 0 8 5"
                    fill="none"
        ...
        <tbody>
          <tr
            v-for="sura in paginatedData"
            :key="sura.id"
            :class="{ 'bg-gray-50 dark:bg-gray-900': sura.selected }"
          >
            <td class="px-4 py-3 border border-gray-100 dark:border-gray-800">
              <label class="flex items-center text-sm font-medium text-gray-700 cursor-pointer select-none dark:text-gray-400">
                <span class="relative">
                  <input type="checkbox" class="sr-only" v-model="sura.selected" @change="updateSelectAll" />
                  <span
                    :class="sura.selected ? 'border-brand-500 bg-brand-500' : 'bg-transparent border-gray-300 dark:border-gray-700'"
                    class="flex h-4 w-4 items-center justify-center rounded-sm border-[1.25px]"
                  >
                    <span :class="sura.selected ? '' : 'opacity-0'">
                      <svg width="12" height="12" viewBox="0 0 12 12" fill="none">
                        <path d="M10 3L4.5 8.5L2 6" stroke="white" stroke-width="1.6666" stroke-linecap="round" stroke-linejoin="round" />
                      </svg>
                    </span>
            ...
            <td class="px-4 py-3 border border-gray-100 dark:border-gray-800">
              <p class="text-gray-700 text-theme-sm dark:text-gray-400">{{ sura.id }}</p>
            </td>
            <td class="px-4 py-3 border border-gray-100 dark:border-gray-800">
              <p class="text-gray-700 text-theme-sm dark:text-gray-400">{{ sura.name_ar }}</p>
            </td>
            <td class="px-4 py-3 border border-gray-100 dark:border-gray-800">
              <p class="text-gray-700 text-theme-sm dark:text-gray-400">{{ sura.name_en }}</p>
            </td>
            <td class="px-4 py-3 border border-gray-100 dark:border-gray-800">
              <p class="text-gray-700 text-theme-sm dark:text-gray-400">{{ sura.ayah_count }}</p>
            </td>
            <td class="px-4 py-3 border border-gray-100 dark:border-gray-800">
              <span
                class="inline-flex items-center justify-center gap-1 rounded-full px-2.5 py-0.5 text-xs font-medium"
                :class="{
                  'bg-green-50 text-green-600 dark:bg-green-500/15 dark:text-green-500': sura.is_active,
                  'bg-error-50 text-error-600 dark:bg-error-500/15 dark:text-error-500': !sura.is_active,
                }"
              >
                {{ sura.is_active ? t('common.active') : t('common.inactive') }}
              </span>
            </td>
            <td class="px-4 py-3 border border-gray-100 dark:border-gray-800">
              <label :for="`toggle-${sura.id}`" class="cursor-pointer">
                <div class="relative">
                  <input
                    :id="`toggle-${sura.id}`"
                    type="checkbox"
                    class="sr-only"
                    :checked="sura.is_active"
                    :disabled="!canEdit"
                    @change="toggleSuraStatus(sura)"
                  />
                  <div
                    class="block h-5 w-9 rounded-full"
                    :class="sura.is_active ? 'bg-brand-500 dark:bg-brand-500' : 'bg-gray-200 dark:bg-white/10'"
                  ></div>
                  <div
                    :class="[
                      sura.is_active ? 'rtl:translate-x-[-100%] ltr:translate-x-full' : 'translate-x-0',
                      'shadow-theme-sm absolute top-0.5 h-4 w-4 rounded-full bg-white duration-200 ease-linear rtl:right-0.5 ltr:left-0.5',
                    ]"
                  ></div>
                </div>
              </label>
            ...
            <td class="px-4 py-3 border border-gray-100 dark:border-gray-800">
              <div class="flex items-center w-full gap-2">
                ...
                <Tooltip :text="t('messages.notAuthorized')" :show="!canDelete">
                  <button
                    :disabled="!canDelete"
                    @click="handleDeleteClick(sura.id)"
  ...
  :message="t('quran_suras.deleteConfirmation')"
...
const canCreate = computed(() => hasAnyPermission(['quran_suras.create', 'quran_suras.store', 'quran_suras.add']))
const canView = computed(() => hasAnyPermission(['quran_suras.view', 'quran_suras.show', 'quran_suras.read']))
const canEdit = computed(() => hasAnyPermission(['quran_suras.update', 'quran_suras.edit']))
const canDelete = computed(() => hasAnyPermission(['quran_suras.delete', 'quran_suras.destroy']))

const { t } = useI18n()
const { success, error } = useNotifications()

const props = defineProps({ suras: Object })

const search = ref('')
const sortColumn = ref('id')
const sortDirection = ref('asc')
const currentPage = ref(props.suras?.current_page ?? 1)
const perPage = ref(props.suras?.per_page ?? 10)
const selectAll = ref(false)

const getSortIconClasses = (column, direction) =>
  sortColumn.value === column && sortDirection.value === direction
    ? 'fill-brand-500 dark:fill-brand-400'
    : 'fill-gray-300 dark:fill-gray-700'

function goToCreate() {
  router.visit(route('admin.quran_suras.create'))
}

function goToView(id) {
  router.visit(route('admin.quran_suras.show', id))
}

function goToEdit(id) {
  router.visit(route('admin.quran_suras.edit', id))
}
...
const suraToDeleteId = ref(null)

function openDeleteModal(id) {
  suraToDeleteId.value = id
...
function closeDeleteModal() {
  ...
  suraToDeleteId.value = null
}

function confirmDelete() {
  if (suraToDeleteId.value) {
    router.delete(route('admin.quran_suras.destroy', suraToDeleteId.value), {
      onSuccess: () => {
  success(t('quran_suras.quranSuraDeletedSuccessfully'))
        closeDeleteModal()
      },
      onError: () => {
  error(t('quran_suras.quranSuraDeletionFailed'))
        closeDeleteModal()
      },
      preserveScroll: true,
    })
...
function toggleSuraStatus(sura) {
  if (!canEdit.value) return
  const wasActive = sura.is_active
  sura.is_active = !wasActive
  const url = wasActive
    ? route('admin.quran_suras.deactivate', { id: sura.id })
    : route('admin.quran_suras.activate', { id: sura.id })
  router.patch(url, {}, {
    preserveState: true,
    preserveScroll: true,
    onError: () => {
      sura.is_active = wasActive
      error(t('quran_suras.quranSuraUpdateFailed'))
    },
  })
}

const filteredData = computed(() => {
  const data = props.suras?.data ?? []
  const searchLower = search.value.trim().toLowerCase()
  if (!searchLower) return [...data]
  return data.filter((sura) => {
    const nameAr = sura.name_ar?.toLowerCase() ?? ''
    const nameEn = sura.name_en?.toLowerCase() ?? ''
    const ayahCount = String(sura.ayah_count ?? '')
    const id = String(sura.id ?? '')
    return nameAr.includes(searchLower) || nameEn.includes(searchLower) || ayahCount.includes(searchLower) || id.includes(searchLower)
  })
})
...
const totalEntries = computed(() => props.suras?.total ?? sortedData.value.length)
const startEntry = computed(() => props.suras?.from ?? (sortedData.value.length ? 1 : 0))
const endEntry = computed(() => props.suras?.to ?? sortedData.value.length)
const totalPages = computed(() => props.suras?.last_page ?? 1)
...
const toggleSelectAll = () => {
  sortedData.value.forEach((sura) => {
    sura.selected = selectAll.value
  })
}

const updateSelectAll = () => {
  const items = sortedData.value
  selectAll.value = items.length > 0 && items.every((sura) => sura.selected)
}
