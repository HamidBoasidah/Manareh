...
const props = defineProps({ term: { type: Object, required: true }, academicYears: { type: Array, required: true } })

const form = useForm({
  _method: 'PUT',
  academic_year_id: props.term.academic_year_id ?? '',
  name: props.term.name ?? '',
  start_date_g: props.term.start_date_g ?? '',
  end_date_g: props.term.end_date_g ?? '',
  start_date_h: props.term.start_date_h ?? '',
  end_date_h: props.term.end_date_h ?? '',
  is_active: !!props.term.is_active,
})

function submit() {
  form.post(route('admin.terms.update', props.term.id), {
    preserveScroll: true,
    onSuccess: () => success(t('terms.termUpdatedSuccessfully')),
    onError: () => error(t('terms.termUpdateFailed')),
  })
}
