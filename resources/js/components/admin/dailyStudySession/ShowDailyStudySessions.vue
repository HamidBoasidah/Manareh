<template>
      ...
      <div class="flex flex-col gap-3 sm:flex-row sm:items-center">
        ...
        <Tooltip :text="t('messages.notAuthorized')" :show="!canCreate">
          ...
          >
            ...
            {{ t('daily_study_sessions.addDailyStudySession') }}
          </button>
        </Tooltip>
      </div>
    </div>
      ...
      <table class="w-full min-w-full">
          ...
          <tr>
            ...
            <th class="px-4 py-3 text-start border border-gray-100 dark:border-gray-800">
              <div class="flex items-center justify-between w-full cursor-pointer" @click="sortBy('circle_name')">
                <p class="font-medium text-gray-700 text-theme-xs dark:text-gray-400">{{ t('daily_study_sessions.circle') }}</p>
            ...
            <th class="px-4 py-3 text-start border border-gray-100 dark:border-gray-800">
              <div class="flex items-center justify-between w-full cursor-pointer" @click="sortBy('session_date_g')">
                <p class="font-medium text-gray-700 text-theme-xs dark:text-gray-400">{{ t('daily_study_sessions.sessionDateG') }}</p>
                <span class="flex flex-col gap-0.5">
                  <svg class="fill-gray-300 dark:fill-gray-700" width="8" height="5" viewBox="0 0 8 5" fill="none">
                    <path d="M4.40962 0.585167C4.21057 0.300808 3.78943 0.300807 3.59038 0.585166L1.05071 4.21327C0.81874 4.54466 1.05582 5 1.46033 5H6.53967C6.94418 5 7.18126 4.54466 6.94929 4.21327L4.40962 0.585167Z" />
                  </svg>
            ...
            <th class="px-4 py-3 text-start border border-gray-100 dark:border-gray-800">
              <p class="font-medium text-gray-700 text-theme-xs dark:text-gray-400">{{ t('daily_study_sessions.sessionDateH') }}</p>
            </th>
        ...
        <tbody>
          <tr
            ...
            :key="session.id"
            :class="{ 'bg-gray-50 dark:bg-gray-900': session.selected }"
          >
            <td class="px-4 py-3 border border-gray-100 dark:border-gray-800">
              <label class="flex items-center text-sm font-medium text-gray-700 cursor-pointer select-none dark:text-gray-400">
                <span class="relative">
                  <input type="checkbox" class="sr-only" v-model="session.selected" @change="updateSelectAll" />
                  <span :class="session.selected ? 'border-brand-500 bg-brand-500' : 'bg-transparent border-gray-300 dark:border-gray-700'" class="flex h-4 w-4 items-center justify-center rounded-sm border-[1.25px]">
                    <span :class="session.selected ? '' : 'opacity-0'">
                      <svg width="12" height="12" viewBox="0 0 12 12" fill="none">
                        <path d="M10 3L4.5 8.5L2 6" stroke="white" stroke-width="1.6666" stroke-linecap="round" stroke-linejoin="round" />
                      </svg>
                    </span>
            ...
            <td class="px-4 py-3 border border-gray-100 dark:border-gray-800">
              <p class="text-gray-700 text-theme-sm dark:text-gray-400">{{ session.circle_name ?? '—' }}</p>
            </td>

            <td class="px-4 py-3 border border-gray-100 dark:border-gray-800">
              <p class="text-gray-700 text-theme-sm dark:text-gray-300">{{ session.session_date_g ?? '—' }}</p>
            </td>

            <td class="px-4 py-3 border border-gray-100 dark:border-gray-800">
              <p class="text-gray-700 text-theme-sm dark:text-gray-400">{{ session.session_date_h ?? '—' }}</p>
            </td>
            ...
            <td class="px-4 py-3 border border-gray-100 dark:border-gray-800">
              <span
                ...
                :class="{
                  'bg-green-50 text-green-600 dark:bg-green-500/15 dark:text-green-500': session.is_active,
                  'bg-error-50 text-error-600 dark:bg-error-500/15 dark:text-error-500': !session.is_active,
                }"
              >
                {{ session.is_active ? t('common.active') : t('common.inactive') }}
              </span>
            </td>

            <td class="px-4 py-3 border border-gray-100 dark:border-gray-800">
              <label :for="'toggle-' + session.id" class="cursor-pointer">
                <div class="relative">
                  <input
                    type="checkbox"
                    ...
                    :disabled="!canEdit"
                    @change="toggleSessionStatus(session)"
                  />
                  <div class="block h-5 w-9 rounded-full" :class="session.is_active ? 'bg-brand-500 dark:bg-brand-500' : 'bg-gray-200 dark:bg-white/10'"></div>
                  <div :class="[session.is_active ? 'rtl:translate-x-[-100%] ltr:translate-x-full' : 'translate-x-0']" class="shadow-theme-sm absolute top-0.5 h-4 w-4 rounded-full bg-white duration-200 ease-linear rtl:right-0.5 ltr:left-0.5"></div>
                </div>
              </label>
            </td>
            ...
            <td class="px-4 py-3 border border-gray-100 dark:border-gray-800">
              <div class="flex items-center w-full gap-2">
                <Tooltip :text="t('messages.notAuthorized')" :show="!canView">
                  <button
                    :disabled="!canView"
                    @click="handleViewClick(session.id)"
                ...
                <Tooltip :text="t('messages.notAuthorized')" :show="!canEdit">
                  <button
                    :disabled="!canEdit"
                    @click="handleEditClick(session.id)"
          ...
          <tr v-if="paginatedData.length === 0">
            <td colspan="7" class="px-4 py-6 text-center text-sm text-gray-500 dark:text-gray-400">
              {{ t('daily_study_sessions.noDailyStudySession') }}
            </td>
          </tr>
        </tbody>
      </table>
  ...
  <DangerAlert
    ...
    :message="t('daily_study_sessions.deleteDailyStudySessionConfirmation')"
...
const canCreate = computed(() => hasAnyPermission(['daily_study_sessions.create', 'daily_study_sessions.store', 'daily_study_sessions.add']))
const canView = computed(() => hasAnyPermission(['daily_study_sessions.view', 'daily_study_sessions.show', 'daily_study_sessions.read']))
const canEdit = computed(() => hasAnyPermission(['daily_study_sessions.update', 'daily_study_sessions.edit']))
const canDelete = computed(() => hasAnyPermission(['daily_study_sessions.delete', 'daily_study_sessions.destroy']))
...
const search = ref('')
const sortColumn = ref('session_date_g')
const sortDirection = ref('desc')
const currentPage = ref(props.dailyStudySessions?.current_page ?? 1)
const perPage = ref(props.dailyStudySessions?.per_page ?? 10)
const selectAll = ref(false)

const isDeleteModalOpen = ref(false)
const sessionToDeleteId = ref(null)

function goToCreate() {
  router.visit(route('admin.daily_study_sessions.create'))
}

function goToView(id) {
  router.visit(route('admin.daily_study_sessions.show', id))
}

function goToEdit(id) {
  router.visit(route('admin.daily_study_sessions.edit', id))
}
...
function confirmDelete() {
  if (!sessionToDeleteId.value) return

  router.delete(route('admin.daily_study_sessions.destroy', sessionToDeleteId.value), {
    preserveScroll: true,
    onSuccess: () => {
      success(t('daily_study_sessions.dailyStudySessionDeletedSuccessfully'))
      closeDeleteModal()
    },
    onError: () => {
      error(t('daily_study_sessions.dailyStudySessionDeletionFailed'))
    },
...
function toggleSessionStatus(session) {
  ...
  session.is_active = !wasActive

  const url = wasActive
    ? route('admin.daily_study_sessions.deactivate', { id: session.id })
    : route('admin.daily_study_sessions.activate', { id: session.id })
...
const filteredData = computed(() => {
  ...
  return (props.dailyStudySessions?.data || [])
    .filter((session) => {
      const circle = session.circle_name?.toLowerCase() || ''
      const dateG = session.session_date_g?.toLowerCase() || ''
      const dateH = session.session_date_h?.toLowerCase() || ''
      return circle.includes(term) || dateG.includes(term) || dateH.includes(term)
    })
    .sort((a, b) => {
...
const totalEntries = computed(() => props.dailyStudySessions?.total || filteredData.value.length)
const startEntry = computed(() => props.dailyStudySessions?.from || (filteredData.value.length ? 1 : 0))
const endEntry = computed(() => props.dailyStudySessions?.to || filteredData.value.length)
const totalPages = computed(() => props.dailyStudySessions?.last_page || 1)
...
watch(
  () => props.dailyStudySessions?.current_page,
  (val) => {
...
watch(
  () => props.dailyStudySessions?.per_page,
  (val) => {
...
const toggleSelectAll = () => {
  filteredData.value.forEach((session) => {
    session.selected = selectAll.value
  })
}

const updateSelectAll = () => {
  const items = filteredData.value
  selectAll.value = items.length > 0 && items.every((session) => session.selected)
}
