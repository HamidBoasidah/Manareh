...
const form = useForm({
  circle_id: defaultCircleId.value,
  session_date_g: '',
  session_date_h: '',
  is_active: true,
})
...
watch(
  () => defaultCircleId.value,
  (newId) => {
    if (!form.circle_id && newId) {
      form.circle_id = newId
    }
  }
)

function submit() {
  form.post(route('admin.daily_study_sessions.store'), {
    onSuccess: () => {
      success(t('daily_study_sessions.dailyStudySessionCreatedSuccessfully'))
      form.reset()
      form.is_active = true
      form.circle_id = defaultCircleId.value
    },
    onError: () => {
      error(t('daily_study_sessions.dailyStudySessionCreationFailed'))
    },
    preserveScroll: true,
  })
}
