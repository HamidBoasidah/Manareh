  ...
  <div class="overflow-hidden">
    ...
    <div class="max-w-full overflow-x-auto">
      <table class="w-full min-w-full">
        <thead>
          <tr>
            ...
            <th class="px-4 py-3 text-start border border-gray-100 dark:border-gray-800">
              <div class="flex items-center justify-between w-full cursor-pointer" @click="sortBy('student_name')">
                <p class="font-medium text-gray-700 text-theme-xs dark:text-gray-400">{{ t('students.studentName') }}</p>
              </div>
            </th>

            <th class="px-4 py-3 text-start border border-gray-100 dark:border-gray-800">
              <div class="flex items-center justify-between w-full cursor-pointer" @click="sortBy('circle_name')">
                <p class="font-medium text-gray-700 text-theme-xs dark:text-gray-400">{{ t('circles.name') }}</p>
              </div>
            </th>

            <th class="px-4 py-3 text-start border border-gray-100 dark:border-gray-800">
              <div class="flex items-center justify-between w-full cursor-pointer" @click="sortBy('exam_type')">
                <p class="font-medium text-gray-700 text-theme-xs dark:text-gray-400">{{ t('exams.type') }}</p>
              </div>
            </th>

            <th class="px-4 py-3 text-start border border-gray-100 dark:border-gray-800">
              <p class="font-medium text-gray-700 text-theme-xs dark:text-gray-400">{{ t('exams.part') }}</p>
            </th>

            <th class="px-4 py-3 text-start border border-gray-100 dark:border-gray-800">
              <p class="font-medium text-gray-700 text-theme-xs dark:text-gray-400">{{ t('exams.date') }}</p>
            </th>
        ...
        <tbody>
          <tr v-for="exam in paginatedData" :key="exam.id" :class="{ 'bg-gray-50 dark:bg-gray-900': exam.selected }">
            <td class="px-4 py-3 border border-gray-100 dark:border-gray-800">
              <label class="flex items-center text-sm font-medium text-gray-700 cursor-pointer select-none dark:text-gray-400">
                <span class="relative">
                  <input type="checkbox" class="sr-only" v-model="exam.selected" @change="updateSelectAll" />
                  <span :class="exam.selected ? 'border-brand-500 bg-brand-500' : 'bg-transparent border-gray-300 dark:border-gray-700'" class="flex h-4 w-4 items-center justify-center rounded-sm border-[1.25px]">
                    <span :class="exam.selected ? '' : 'opacity-0'">
                      <svg width="12" height="12" viewBox="0 0 12 12" fill="none">
                        <path d="M10 3L4.5 8.5L2 6" stroke="white" stroke-width="1.6666" stroke-linecap="round" stroke-linejoin="round"/>
                      </svg>
                    </span>
            ...
            <td class="px-4 py-3 border border-gray-100 dark:border-gray-800">
              <p class="text-gray-700 text-theme-sm dark:text-gray-400">{{ exam.student?.user?.name ?? exam.student_name ?? '—' }}</p>
            </td>

            <td class="px-4 py-3 border border-gray-100 dark:border-gray-800">
              <p class="text-gray-700 text-theme-sm dark:text-gray-400">{{ exam.circle?.name ?? exam.circle_name ?? '—' }}</p>
            </td>

            <td class="px-4 py-3 border border-gray-100 dark:border-gray-800">
              <p class="text-gray-700 text-theme-sm dark:text-gray-400">{{ t('exams.types.' + exam.exam_type) }}</p>
            </td>

            <td class="px-4 py-3 border border-gray-100 dark:border-gray-800">
              <p class="text-gray-700 text-theme-sm dark:text-gray-400">{{ exam.exam_type === 'stage' ? exam.stage ?? '—' : exam.juzz ?? '—' }}</p>
            </td>

            <td class="px-4 py-3 border border-gray-100 dark:border-gray-800">
              <p class="text-gray-700 text-theme-sm dark:text-gray-400">{{ exam.exam_date_g ?? exam.exam_date_h ?? '—' }}</p>
            </td>
            ...
            <td class="px-4 py-3 border border-gray-100 dark:border-gray-800">
              <label :for="'toggle-' + exam.id" class="cursor-pointer">
                <div class="relative">
                  <input type="checkbox" :id="'toggle-' + exam.id" class="sr-only" :checked="exam.is_active" @change="toggleExamStatus(exam)" />
                  <div class="block h-5 w-9 rounded-full" :class="exam.is_active ? 'bg-brand-500 dark:bg-brand-500' : 'bg-gray-200 dark:bg-white/10'"></div>
                  <div :class="[exam.is_active ? 'rtl:translate-x-[-100%] ltr:translate-x-full' : 'translate-x-0']" class="shadow-theme-sm absolute top-0.5 h-4 w-4 rounded-full bg-white duration-200 ease-linear rtl:right-0.5 ltr:left-0.5"></div>
                </div>
              </label>
            </td>
            ...
            <td class="px-4 py-3 border border-gray-100 dark:border-gray-800">
              <div class="flex items-center w-full gap-2">
                ...
                <Tooltip :text="t('messages.notAuthorized')" :show="!canDelete">
                  <button
                    :disabled="!canDelete"
                    @click="handleDeleteClick(exam.id)"
          ...
          <tr v-if="paginatedData.length === 0">
            <td colspan="9" class="px-4 py-6 text-center text-sm text-gray-500 dark:text-gray-400">{{ t('exams.noExam') }}</td>
          </tr>
        </tbody>
      </table>
    </div>
    ...
    <DangerAlert :isOpen="isDeleteModalOpen" :title="t('messages.areYouSure')" :message="t('messages.deleteExamConfirmation')" @close="closeDeleteModal" @confirm="confirmDelete" />
  </div>
</template>
...
const canCreate = computed(() => hasAnyPermission(['exams.create', 'exams.store', 'exams.add']))
const canView   = computed(() => hasAnyPermission(['exams.view', 'exams.show', 'exams.read']))
const canEdit   = computed(() => hasAnyPermission(['exams.update', 'exams.edit']))
const canDelete = computed(() => hasAnyPermission(['exams.delete', 'exams.destroy']))
...
const search         = ref('')
const sortColumn     = ref('exam_date_g')
const sortDirection  = ref('desc')
const currentPage    = ref(props.exams?.current_page ?? 1)
const perPage        = ref(props.exams?.per_page ?? 10)
const selectAll      = ref(false)

function goToCreate()      { router.visit(route('admin.exams.create')) }
function goToView(id)      { router.visit(route('admin.exams.show', id)) }
function goToEdit(id)      { router.visit(route('admin.exams.edit', id)) }
...
function openDeleteModal(id)  { itemToDeleteId.value = id; isDeleteModalOpen.value = true }
function handleDeleteClick(id){ if (!canDelete.value) return; openDeleteModal(id) }
function closeDeleteModal()   { isDeleteModalOpen.value = false; itemToDeleteId.value = null }

function confirmDelete() {
  if (itemToDeleteId.value) {
    router.delete(route('admin.exams.destroy', itemToDeleteId.value), {
      onSuccess: () => { success(t('exams.examDeletedSuccessfully')); closeDeleteModal() },
      onError:   () => { error(t('exams.examDeletionFailed'));       closeDeleteModal() },
      preserveScroll: true,
    })
  }
}

function toggleExamStatus(item) {
  const wasActive = item.is_active
  item.is_active = !wasActive
  const url = wasActive
    ? route('admin.exams.deactivate', { id: item.id })
    : route('admin.exams.activate',   { id: item.id })
  router.patch(url, {}, {
    preserveState: true,
    preserveScroll: true,
    onError: () => { item.is_active = wasActive },
  })
}

const filteredData = computed(() => {
  const searchLower = (search.value || '').toLowerCase()
  return (props.exams?.data || [])
    .filter((d) => {
      const student = (d.student?.user?.name || d.student_name || '')
      const circle = (d.circle?.name || d.circle_name || '')
      return student.toLowerCase().includes(searchLower) || circle.toLowerCase().includes(searchLower)
    })
    .sort((a, b) => {
...
const totalEntries = computed(() => props.exams?.total || filteredData.value.length)
const startEntry   = computed(() => props.exams?.from  || 1)
const endEntry     = computed(() => props.exams?.to    || filteredData.value.length)
const totalPages   = computed(() => props.exams?.last_page || 1)
...
watch(() => props.exams?.current_page, (val) => { currentPage.value = typeof val === 'number' ? val : 1 })
watch(() => props.exams?.per_page, (val) => { if (typeof val === 'number') perPage.value = val })
...
const updateSelectAll = () => { const items = filteredData.value; selectAll.value = items.length > 0 && items.every((d) => d.selected) }
