<template>
      ...
      <table class="w-full min-w-full">
        <thead>
          <tr>
            ...
            <th class="px-4 py-3 text-start border border-gray-100 dark:border-gray-800">
              <p class="font-medium text-gray-700 text-theme-xs dark:text-gray-400">{{ t('circles.mosque') }}</p>
            </th>

            <th class="px-4 py-3 text-start border border-gray-100 dark:border-gray-800">
              <p class="font-medium text-gray-700 text-theme-xs dark:text-gray-400">{{ t('circles.classification') }}</p>
            </th>

            <th class="px-4 py-3 text-start border border-gray-100 dark:border-gray-800">
              <div class="flex items-center justify-between w-full cursor-pointer" @click="sortBy('capacity')">
                <p class="font-medium text-gray-700 text-theme-xs dark:text-gray-400">{{ t('circles.capacity') }}</p>
                <span class="flex flex-col gap-0.5">
                  <svg class="fill-gray-300 dark:fill-gray-700" width="8" height="5" viewBox="0 0 8 5" fill="none"><path d="M4.40962 0.585167C4.21057 0.300808 3.78943 0.300807 3.59038 0.585166L1.05071 4.21327C0.81874 4.54466 1.05582 5 1.46033 5H6.53967C6.94418 5 7.18126 4.54466 6.94929 4.21327L4.40962 0.585167Z"/></svg>
                  <svg class="fill-gray-300 dark:fill-gray-700" width="8" height="5" viewBox="0 0 8 5" fill="none"><path d="M4.40962 4.41483C4.21057 4.69919 3.78943 4.69919 3.59038 4.41483L1.05071 0.786732C0.81874 0.455343 1.05582 0 1.46033 0H6.53967C6.94418 0 7.18126 0.455342 6.94929 0.786731L4.40962 4.41483Z"/></svg>
                </span>
        ...
        <tbody>
          <tr
            v-for="circle in paginatedData"
            :key="circle.id"
            :class="{ 'bg-gray-50 dark:bg-gray-900': circle.selected }"
          >
            <td class="px-4 py-3 border border-gray-100 dark:border-gray-800">
              <label class="flex items-center text-sm font-medium text-gray-700 cursor-pointer select-none dark:text-gray-400">
                <span class="relative">
                  <input type="checkbox" class="sr-only" v-model="circle.selected" @change="updateSelectAll" />
                  <span
                    :class="circle.selected ? 'border-brand-500 bg-brand-500' : 'bg-transparent border-gray-300 dark:border-gray-700'"
                    class="flex h-4 w-4 items-center justify-center rounded-sm border-[1.25px]"
                  >
                    <span :class="circle.selected ? '' : 'opacity-0'">
                      <svg width="12" height="12" viewBox="0 0 12 12" fill="none">
                        <path d="M10 3L4.5 8.5L2 6" stroke="white" stroke-width="1.6666" stroke-linecap="round" stroke-linejoin="round"/>
                      </svg>
                    </span>
            ...
            <td class="px-4 py-3 border border-gray-100 dark:border-gray-800">
              <p class="text-gray-700 text-theme-sm dark:text-gray-400">{{ circle.name }}</p>
            </td>

            <td class="px-4 py-3 border border-gray-100 dark:border-gray-800">
              <p class="text-gray-700 text-theme-sm dark:text-gray-400">{{ circle.mosque_name ?? '—' }}</p>
            </td>

            <td class="px-4 py-3 border border-gray-100 dark:border-gray-800">
              <p class="text-gray-700 text-theme-sm dark:text-gray-400">{{ circle.classification_name ?? '—' }}</p>
            </td>

            <td class="px-4 py-3 border border-gray-100 dark:border-gray-800">
              <p class="text-gray-700 text-theme-sm dark:text-gray-400">{{ circle.capacity ?? '—' }}</p>
            </td>
            ...
            <td class="px-4 py-3 border border-gray-100 dark:border-gray-800">
              <label :for="'toggle-' + circle.id" class="cursor-pointer">
                <div class="relative">
                  <input
                    type="checkbox"
                    :id="'toggle-' + circle.id"
                    ...
                    @change="toggleCircleStatus(circle)"
                  />
                  <div class="block h-5 w-9 rounded-full" :class="circle.is_active ? 'bg-brand-500 dark:bg-brand-500' : 'bg-gray-200 dark:bg-white/10'"></div>
                  <div :class="[circle.is_active ? 'rtl:translate-x-[-100%] ltr:translate-x-full' : 'translate-x-0']" class="shadow-theme-sm absolute top-0.5 h-4 w-4 rounded-full bg-white duration-200 ease-linear rtl:right-0.5 ltr:left-0.5"></div>
                </div>
              </label>
            </td>
            ...
            <td class="px-4 py-3 border border-gray-100 dark:border-gray-800">
              <div class="flex items-center w-full gap-2">
                <Tooltip :text="t('messages.notAuthorized')" :show="!canView">
                  <button
                    :disabled="!canView"
                    @click="handleViewClick(circle.id)"
                ...
                <Tooltip :text="t('messages.notAuthorized')" :show="!canEdit">
                  <button
                    :disabled="!canEdit"
                    @click="handleEditClick(circle.id)"
                ...
                <Tooltip :text="t('messages.notAuthorized')" :show="!canDelete">
                  <button
                    :disabled="!canDelete"
                    @click="handleDeleteClick(circle.id)"
  ...
  <DangerAlert
    ...
    :message="t('messages.deleteCircleConfirmation')"
...
const canCreate = computed(() => hasAnyPermission(['circles.create', 'circles.store', 'circles.add']))
const canView   = computed(() => hasAnyPermission(['circles.view', 'circles.show', 'circles.read']))
const canEdit   = computed(() => hasAnyPermission(['circles.update', 'circles.edit']))
const canDelete = computed(() => hasAnyPermission(['circles.delete', 'circles.destroy']))

const { t } = useI18n()
const { success, error } = useNotifications()

const props = defineProps({ circles: Object })

const search        = ref('')
const sortColumn    = ref('name')
const sortDirection = ref('asc')
const currentPage   = ref(props.circles?.current_page ?? 1)
const perPage       = ref(props.circles?.per_page ?? 10)
const selectAll     = ref(false)

const isDeleteModalOpen = ref(false)
const circleToDeleteId  = ref(null)

function goToCreate()      { router.visit(route('admin.circles.create')) }
function goToView(id)      { router.visit(route('admin.circles.show', id)) }
function goToEdit(id)      { router.visit(route('admin.circles.edit', id)) }
...
function openDeleteModal(id)  { circleToDeleteId.value = id; isDeleteModalOpen.value = true }
function handleDeleteClick(id){ if (!canDelete.value) return; openDeleteModal(id) }
function closeDeleteModal()   { isDeleteModalOpen.value = false; circleToDeleteId.value = null }

function confirmDelete() {
  if (circleToDeleteId.value) {
    router.delete(route('admin.circles.destroy', circleToDeleteId.value), {
      onSuccess: () => { success(t('circles.circleDeletedSuccessfully')); closeDeleteModal() },
      onError:   () => { error(t('circles.circleDeletionFailed')); closeDeleteModal() },
      preserveScroll: true,
    })
  }
}

function toggleCircleStatus(circle) {
  const previous = circle.is_active
  circle.is_active = !previous
  const url = previous
    ? route('admin.circles.deactivate', { id: circle.id })
    : route('admin.circles.activate',   { id: circle.id })
  router.patch(url, {}, {
    preserveState: true,
    preserveScroll: true,
    onError: () => { circle.is_active = previous },
  })
}

const filteredData = computed(() => {
  const term = (search.value || '').toLowerCase()
  const items = (props.circles?.data || [])
    .filter((circle) => {
      const name = circle.name?.toLowerCase() || ''
      const mosque = circle.mosque_name?.toLowerCase() || ''
      const classification = circle.classification_name?.toLowerCase() || ''
      const notes = circle.notes?.toLowerCase() || ''
      return name.includes(term) || mosque.includes(term) || classification.includes(term) || notes.includes(term)
    })
  ...
  return items.sort((a, b) => {
    if (column === 'capacity') {
      const aVal = Number(a.capacity) || 0
      const bVal = Number(b.capacity) || 0
    ...
    return valueA.localeCompare(valueB, undefined, { sensitivity: 'base' }) * modifier
  })
})

const paginatedData = computed(() => filteredData.value)

const totalEntries = computed(() => props.circles?.total || filteredData.value.length)
const startEntry   = computed(() => props.circles?.from  || (filteredData.value.length ? 1 : 0))
const endEntry     = computed(() => props.circles?.to    || filteredData.value.length)
const totalPages   = computed(() => props.circles?.last_page || 1)
...
watch(() => props.circles?.current_page, (val) => {
  currentPage.value = typeof val === 'number' ? val : 1
})

watch(() => props.circles?.per_page, (val) => {
...
const toggleSelectAll = () => {
  filteredData.value.forEach((circle) => {
    circle.selected = selectAll.value
  })
}

const updateSelectAll = () => {
  const items = filteredData.value
  selectAll.value = items.length > 0 && items.every((circle) => circle.selected)
}
