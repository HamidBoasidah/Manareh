      ...
      <div class="p-4 sm:p-6 dark:border-gray-800">
        <div class="grid grid-cols-1 gap-5 md:grid-cols-2">
          <div class="md:col-span-2">
            <label for="circle-name" class="mb-1.5 block text-sm font-medium text-gray-700 dark:text-gray-400">
              {{ t('circles.name') }}
            </label>
            <input
              v-model="form.name"
          ...
          <div>
            ...
            <div class="relative z-20 bg-transparent">
              <select
                ...
                class="dark:bg-dark-900 shadow-theme-xs focus:border-brand-300 focus:ring-brand-500/10 dark:focus:border-brand-800 h-11 w-full appearance-none rounded-lg border border-gray-300 bg-transparent bg-none px-4 py-2.5 pr-11 text-sm text-gray-800 placeholder:text-gray-400 focus:ring-3 focus:outline-hidden dark:border-gray-700 dark:bg-gray-900 dark:text-white/90 dark:placeholder:text-white/30"
                :class="{ 'text-gray-800 dark:text-white/90': form.classification_id }"
              >
                <option value="" class="text-gray-700 dark:bg-gray-900 dark:text-gray-400">
                  {{ t('circles.selectClassification') }}
                </option>
                <option
            ...
            <p v-if="form.errors.classification_id" class="mt-1 text-sm text-error-500">{{ form.errors.classification_id }}</p>
          </div>

          <div>
            <label for="circle-capacity" class="mb-1.5 block text-sm font-medium text-gray-700 dark:text-gray-400">
              {{ t('circles.capacity') }}
            </label>
            <input
              ...
              min="0"
              step="1"
              :placeholder="t('circles.capacity')"
              class="dark:bg-dark-900 shadow-theme-xs focus:border-brand-300 focus:ring-brand-500/10 dark:focus:border-brand-800 h-11 w-full rounded-lg border border-gray-300 bg-transparent px-4 py-2.5 text-sm text-gray-800 placeholder:text-gray-400 focus:ring-3 focus:outline-hidden dark:border-gray-700 dark:bg-gray-900 dark:text-white/90 dark:placeholder:text-white/30"
            />
            <p v-if="form.errors.capacity" class="mt-1 text-sm text-error-500">{{ form.errors.capacity }}</p>
          </div>

          <div class="md:col-span-2">
            <label for="circle-notes" class="mb-1.5 block text-sm font-medium text-gray-700 dark:text-gray-400">
              {{ t('circles.notes') }}
            </label>
            <textarea
              v-model="form.notes"
...
const form = useForm({
  ...
  mosque_id: props.circle.mosque_id ?? '',
  classification_id: props.circle.classification_id ?? '',
  capacity: props.circle.capacity ?? '',
  notes: props.circle.notes ?? '',
  is_active: !!props.circle.is_active,
})

function update() {
  form.post(route('admin.circles.update', props.circle.id), {
    onSuccess: () => {
      success(t('circles.circleUpdatedSuccessfully'))
    },
    onError: () => {
      error(t('circles.circleUpdateFailed'))
    },
    preserveScroll: true,
  })
}
